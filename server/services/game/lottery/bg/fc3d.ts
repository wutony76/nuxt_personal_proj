 ...
 *   來源（bglottery fc3d）只有官方盤、沒有信用盤，也沒有任何彩池／爆池 UI 痕跡，
 *   是本專案目前唯一「官方盤單盤口」的形狀：
 *     判定／賠率／複式驗證 → 取 sscOf.ts 的「一般分頁」半套（固定賠率、逐注鎖賠率），
 *                            但**不含** sscOf 的彩池分頁分支（fc3d 一個吃池的分頁都沒有）。
 ...
 *   一注一碼後送上來，伺端逐注用 fc3dHasBetCode() 驗（複式分頁改驗「前綴符合 + 可判定」）。
 *   ⚠️ 伺端不信任前端送的注數與分頁 id：注數超過 FC3D_MAX_COMBO 整筆拒絕，
 *      分頁 id 一律以 config（findFc3dTab）重解析，賠率由 fc3dTabOddsOf() 重算後才鎖進注單。
 */
...
type UserStoreLike = { userId?: string; coin?: number; fc3dRecord?: UserRecord }
...
function _fc3dRandomOpenCode(): string[] {
...
export default class FC3D extends LOTTERY_BASE {
  ...
  constructor() {
    ...
    Object.assign(this.handle, {
      // 福彩3D開獎：3 個號碼（可重複，0~9），比照 eggs 的三球，不像時時彩是 5 位
      randomOpenCode: () => _fc3dRandomOpenCode(),
      ...
      ensureUserRecord: (user: UserStoreLike) => {
        // 與其他彩種分開存（fc3dRecord），各盤口的注單紀錄互不干擾
        if (!user.fc3dRecord) {
          user.fc3dRecord = { balanceChanges: [], betHistory: [], claimableIssues: [], updatedAt: Date.now() }
        }
        if (!Array.isArray(user.fc3dRecord.balanceChanges)) user.fc3dRecord.balanceChanges = []
        if (!Array.isArray(user.fc3dRecord.betHistory)) user.fc3dRecord.betHistory = []
        if (!Array.isArray(user.fc3dRecord.claimableIssues)) user.fc3dRecord.claimableIssues = []
        user.fc3dRecord.updatedAt = Date.now()
        return user.fc3dRecord
      },
       ...
       *
       * 官方盤只有一種注碼形狀（字串），且 fc3d 沒有彩池分頁，所以比 SSC-OF 更單純：
       *   單選分頁（定位膽）→ 注碼要在 groupList 內
       *   複式分頁          → 前綴符合該分頁的 combo 規則且能被 fc3d-of.ts 判定
      ...
      validateBetQuota: (input: { issue: string; userId: string; amount: number; groups: Group[] }) => {
        ...
        ;(Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
          ...
          const tab = findFc3dTab(playKey, group?.selectTabId)
          ...
          const quota = fc3dQuotaOf(playKey, safeTabId)
          const playList = Array.isArray(group?.playList) ? group.playList : []
          // 複式展開的上限：前端的 fc3dComboCodes() 已擋過一次，這裡是伺端的第二道
          if (playList.length > FC3D_MAX_COMBO) {
            this.handle.rejectBet(`${tabName} 單次最多 ${_money(FC3D_MAX_COMBO)} 注，本次 ${_money(playList.length)} 注`)
          }
          playList.forEach((play) => {
            const betCode = _resolveBetCode(play)
            if (!betCode || !fc3dHasBetCode(playKey, safeTabId, betCode)) {
        ...
        newByTab.forEach(({ playKey, coin: newCoin }, tabId) => {
          const quota = fc3dQuotaOf(playKey, tabId)
          ...
          if (used + newCoin > quota.issue.max) {
            const tabName = findFc3dTab(playKey, tabId)?.tabName ?? String(tabId)
      ...
      buildOrderRows: (input: { issue: string; userId: string; amount: number; groups: Group[] }): BetOrderRow[] => {
        ...
        ;(Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
          ...
          const tabId = Number(findFc3dTab(playKey, group?.selectTabId)?.tabId ?? 0)
          const playList = Array.isArray(group?.playList) ? group.playList : []
          ...
          playList.forEach((play, index) => {
            ...
            rows.push({
              ...
              odds: fc3dTabOddsOf(playKey, tabId, betCode),
              tab_id: tabId
            })
          })
        })
      ...
      settleIssuePrize: (issue: string, openCode: string[]) => {
        ...
        oddsOrders.forEach((row) => {
          ...
          const judged = judgeFc3dBet(betCode, codes, coin, lockedOdds)
