 ...
 * 快3 頁首
 *
 * 版面與樣式參照 6hc-of 的 Header（app/components/lottery/bg/6hc/of/block/Header.vue）：
 *   .header-warp  4px 深紅外框 + 金色漸層頂條
 *   .left         .info（毛筆大標題 + 玩法 + LOTTERY_ID）＋ .info-bonus（獎金框）
 *   .right .inner .timer（期別／狀態／倒數，佔 30% 淡紅底）＋ .open-code（可點的開獎區）
 *
 * K3 專屬差異：開獎球換成 3 顆骰子，獎金框改顯示共用彩池（信用盤與官方盤共同累積）。
 */
 ...
 *
 * 56 種注碼 × 216 種開獎結果全部窮舉，不寫死數字 ——
 * 分層或骰子規則一改，這裡自動跟上。命中 1 顆就有三獎，故門檻是 ≥ 1。
 * ⚠️ 這是「所有注碼平均」；實際單注差很多（1-2-3 這種三顆不同的約 87.5%，
 *    1-1-1 只有 42.1%），所以標題只寫機率、細節留給玩法說明頁。
 * 常數運算，放模組層算一次（12096 次迴圈），不隨 render 重算。
 */
const K3_OF_WIN_RATE = (() => {
  const picks = k3OfAllPicks()
  const outcomes = k3AllOutcomes()
  let win = 0
  let total = 0
  picks.forEach((pick) => outcomes.forEach((dice) => {
    total += 1
    // 回 null 代表注碼不合法，這裡窮舉出來的一定合法，保守當作沒中
    if ((k3OfMatchCount(pick, dice) ?? 0) > 0) win += 1
  }))
...
/** 已開獎的那一期骰子（未開獎的當期不提前顯示，未開出時給 3 個問號） */
...
const dice = computed(() => (openCode.value.length > 0 ? openCode.value : [0, 0, 0]))
    ...
    <div class="right">
      <div class="inner">
        ...
        <div class="open-code" role="button" tabindex="0" @click="emit('open-opencode-dialog')"
          ...
          <div class="main">
            <!-- 6hc 的球下方有「攪出次數」所以 legend 有第二行；K3 骰子沒有逐顆統計，只留標題 -->
