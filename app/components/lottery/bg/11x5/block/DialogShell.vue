...
<template>
  <div v-if="props.visible" class="x5-dialog-mask" @click.self="emit('close')">
    <section class="x5-dialog lottery-scrollbar" :style="props.width ? `width: ${props.width}` : undefined">
      <header class="x5-dialog-header">
        <h3>{{ props.title }}</h3>
        <button type="button" class="close-btn" aria-label="關閉" @click="emit('close')">×</button>
      </header>
      <div class="x5-dialog-body">
        <slot />
      </div>
    </section>
  </div>
