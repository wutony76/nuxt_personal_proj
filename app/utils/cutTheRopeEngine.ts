...
export const CUT_THE_ROPE_LEVELS: CutTheRopeLevelDef[] = [
  {
    // 起手偏移 6 度（原本正下方垂直懸掛，完全靜止不動），讓糖果一放手就有輕微鐘擺晃動。
    // 原本用 8 度，但終點判定收緊到「中心要進終點圓內」（GOAL_RADIUS=32px）後，8 度累積的
    // 擺盪橫向飄移會讓部分剪繩時機的最終落點剛好差 0.1~6px 沒進到範圍內，剪繩時機逐 tick
    // 掃描 0~500 tick 出現 44 次「看起來很接近終點卻沒過關」的真實落空；改成 6 度後幅度壓
    // 小，同樣掃描全部 501 個時間點都能過關且收集到星星，不影響「直接剪」的入門難度。
    candyStart: { x: 193, y: 179 },
  ...
  },
  {
    ...
    // 兩顆星星刻意放在「附著擺盪」時就會經過的弧線座標上，讓玩家不剪繩單純擺盪也能收集到。
    // 終點則改放在「兩顆星都掃過後，在擺盪弧線最右側附近剪繩」才會飛到的自由落體路徑上
    // （見開發時用 tracer 腳本描出的軌跡：約 tick 32~72 剪繩才過關），避免終點直接落在弧線本身
    // 導致完全不用剪繩、光靠擺盪就自動過關。
