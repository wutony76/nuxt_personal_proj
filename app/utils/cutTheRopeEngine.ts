...
export const CUT_THE_ROPE_LEVELS: CutTheRopeLevelDef[] = [
  {
    // 起手偏移 8 度（原本正下方垂直懸掛，完全靜止不動），讓糖果一放手就有明顯的鐘擺晃動。
    // 終點判定收緊到「中心要進終點圓內」（GOAL_RADIUS=32px）後，8 度累積的擺盪橫向飄移會讓
    // 部分剪繩時機的最終落點差 0.1~6px 沒進到範圍內、真的沒過關（不是判定 bug，是這個角度
    // 下确實沒進去），不強求每個剪繩時機都一定過關——只要求判定本身準確，故意留著較明顯的
    // 8 度晃動效果。
