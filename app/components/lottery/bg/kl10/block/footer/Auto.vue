...
/**
 * 快樂十分自動下注（版面與 k3 的 cd/block/footer/Auto.vue 同一套）
 *
 * 由 app.vue 的 BgAutoPanel 統一渲染，頁面進入時以 useBgAutoActive 啟用。
 * 每期開盤時從當前分頁的注項池隨機取 N 個各下 M 元；以期數為單位記錄，避免同期重複觸發。
 * ⚠️ 走 fetch.autoBets（直接組 payload），不動使用者手動填的注項；
 *    但送單成功後會與手動下注一樣清空選取。
 * ⚠️ 任選分頁沒有注項池（看板是號碼池）—— autoBets 在那個模式改成「機選滿最少選號數 → 送複式」，
 *    因為 minChosen 就等於一注的碼數，展開後固定是 1 注，所以注數欄位在那裡不給填。
 */
const {
...
const _actions = {
  ...
  autoBet: async (issue: string) => {
    ...
    try {
      const result = await mxFetch.autoBets({ count: autoBetCount.value, amount: state.betAmount })
      if (result?.ok) {
...
watch([() => mxState.select, () => mxState.selectTabId], () => {
  state.lastIssue = ''
})
</script>
...
.auto-warp {
  ...
  .control-auto {
    ...
    .auto-coin {
      ...
      /* 任選：注數固定 1，用純文字取代輸入框（樣式對齊 .coin-input） */
      .auto-fixed {
        width: 72px;
        border: 1px solid $c-border;
        border-radius: 4px;
