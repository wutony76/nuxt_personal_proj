...
## Addendum：既有爆池（Header「總彩池」）也補上起始池底

實作完成後，使用者發現 Header 上方顯示的「總彩池」一進遊戲仍是 0（要下注才開始累加）——
這綁定的是**既有的爆池**（`buildJackpotShares`），跟本次新增的彩池玩法彩金池是兩個不同的池，
使用者確認也要幫這個既有爆池補上非 0 的起始池底。

### 為什麼不能直接複製彩池玩法的「每期都加回去」機制

彩池玩法的池底公式（`LOTTERY_BASE.jackpotCalc`）每期都會把 `poolBase` 重新乘上 0.55 的
阻尼係數再併入 `distributablePool`，這個 <1 的係數保證即使从不觸發，池子也會收斂在有界範圍內
（跟 k3-of 的既有設計同一套數學）。但既有爆池的公式（`buildJackpotShares`）沒有這個阻尼——
未觸發期是整池 100% 原封滾存。如果照樣讓 `base` 每期都重新加回去，滾存會隨期數線性、無界成長，
幾百期後總彩池會膨脹成天文數字。

### 改用一次性 seed，之後照既有機制自然演化

```ts
// eggs.ts / kl10.ts constructor
this.carryJackpot = LOTTERY_BASE.jackpotBase(EGGS_JACKPOT_BASE_MIN, EGGS_JACKPOT_BASE_MAX)
```
只在開站時把 `carryJackpot` seed 成一個介於 110,000~450,000 的隨機值（範圍比照彩池玩法但獨立
宣告常數），之後完全走既有的抽水／觸發／滾存邏輯，不再另外處理——池底自然融進滾存演化，
不會再被單獨識別出來，也因此沒有「重骰門檻」的必要（既有機制本身就允許滾存自然消長）。

實測：重啟服務後，KL10／EGGS 的「總彩池」立即顯示非 0（見畫面截圖 115,221.00 / 341,646.00），
「預估發放」＝總彩池 × 50% 正確跟著算。
