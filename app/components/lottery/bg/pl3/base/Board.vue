 ...
 * 排列3投注看板
 *
 * 一個元件要應付 3 種分頁型態（見 pl3of/plays.js 的分工）：
 *   單選分頁（定位膽）      —— 與 eggs 同款的「號碼｜金額」表格，逐項填金額
 *   複式分頁（其餘 4 個）  —— 每個位置／每組一列可選值（號碼、和值或面），多選；
 *                            注數＝各列選數相乘（組選類是組合數，見 usePl3 的 rawComboCount）
...
<template>
  <div class="pl3-board">
    ...
    <template v-else-if="combo">
      <div v-for="group in comboGroups" :key="`pl3-pos-${group.pos}`" class="pick-row">
        <div class="pick-head">
    ...
    <template v-else>
      ...
      <div v-for="group in singleGroups" :key="`pl3-group-${group.groupName}`" class="play-group">
