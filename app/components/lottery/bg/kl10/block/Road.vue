...
/**
 * 快樂十分路珠（版面／邏輯比照 `k3/block/Road.vue`）
 *
 * ⚠️ 與 k3 / eggs 的差異：快樂十分的走勢有四個角度 ——
 *    總和大小（≥84 大）、總和單雙，以及看板獨有的上下盤／奇偶盤（4:4 時是「和盤」）。
 *    上下盤與奇偶盤會出現第三種結果（和），所以路珠比 eggs 多一個 tie 色。
 */
...
type Outcome = 'big' | 'small' | 'odd' | 'even' | 'up' | 'down' | 'oddZone' | 'evenZone' | 'tie'

const LABELS: Record<Outcome, string> = {
  big: '大', small: '小', odd: '單', even: '雙',
  up: '上', down: '下', oddZone: '奇', evenZone: '偶', tie: '和'
}
/** 大／單／上／奇走紅、小／雙／下／偶走藍、和盤走灰（與 Header、開獎歷史同一套配色） */
...
const _handlers = {
  outcomeOf: (openCode: string[], dimension: DimensionKey): Outcome | null => {
    const nums = kl10NumbersOf(Array.isArray(openCode) ? openCode : [])
    if (!nums) return null
    if (dimension === 'size') return kl10SumOf(nums) >= KL10_SUM_BIG_LINE ? 'big' : 'small'
    if (dimension === 'parity') return kl10SumOf(nums) % 2 === 1 ? 'odd' : 'even'
    if (dimension === 'zone') {
      const zone = kl10ZoneOf(nums)
      return zone === '上盤' ? 'up' : zone === '下盤' ? 'down' : 'tie'
    }
    const parityZone = kl10ParityZoneOf(nums)
...
<template>
  <section class="block-main kl10-road">
    <div class="road-head">
    ...
    <p class="road-note">
      ※ 總和 ≥{{ KL10_SUM_BIG_LINE }} 為大；上下盤比「1~{{ KL10_HALF_LINE }} 的個數」、
      奇偶盤比「奇數的個數」，4:4 為和；往左拉可看更早的走勢
    </p>
  </section>
</template>

<style scoped lang="scss">
/* 卡片外框由全域 .lottery-kl10 .block-main 提供（同開獎歷史、當前注項） */
.kl10-road {
  min-width: 0;
  display: flex;
