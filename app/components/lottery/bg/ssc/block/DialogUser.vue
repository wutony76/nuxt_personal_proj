...
const { userRecord: mxRecord, wallet: mxWallet, isCd, fetch: mxFetch } = useSsc()
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="item in mxRecord.claimableIssues" :key="item.issue">
            ...
            <td>
              <span class="du-balls">
                <Ball v-for="(code, idx) in (item.openCode ?? [])" :key="idx" :digit="code" size="xs" />
              </span>
            </td>
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="row in mxRecord.betHistory" :key="row.orderId">
            ...
            <td>
              <!-- 兩個盤口都是固定賠率（官方盤沒有 pk10 那種彩池分層），一律顯示鎖進注單的賠率 -->
            ...
            <td>
              <span v-if="row.openCode?.length" class="du-balls">
