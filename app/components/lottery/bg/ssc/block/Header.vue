 ...
 *
 * SSC 專屬差異：開獎是 5 個 0~9 的號碼（可重複），沒有名次也沒有車號互異，
 * 所以開獎區改成一排 5 顆號碼球；兩面改看「總和」，分界讀 SSC_SUM_BIG_LINE。
 */
...
const openCode = computed(() => {
  ...
  return codes.length === SSC_BALL_COUNT ? codes : []
})
/** 總和與它的兩面（分界讀 SSC_SUM_BIG_LINE，不寫死 23） */
const sum = computed(() => mxActions.sumOf(openCode.value))
const isBig = computed(() => sum.value >= SSC_SUM_BIG_LINE)
 ...
 * ⚠️ 與 pk10 不同：時時彩號碼可以重複，翻號直接各自亂數即可，不必洗牌。
 */
const ROLL_INTERVAL_MS = 90
const anim = reactive({
  rolling: false,
  rollBalls: Array.from({ length: SSC_BALL_COUNT }, () => 0) as number[],
  revealToken: 0
})
...
const _anim = {
  start: () => {
    ...
    rollTimer = setInterval(() => {
      anim.rollBalls = Array.from({ length: SSC_BALL_COUNT }, () => Math.floor(Math.random() * (SSC_DIGIT_MAX + 1)))
    }, ROLL_INTERVAL_MS)
  },
  stop: () => {
...
/** 球位短標籤（球下方那一行；沿用 SSC_BALL_NAMES 但只取「第X」兩字，全名會太寬） */
const ballLabels = SSC_BALL_NAMES.map((name) => name.replace('球', ''))
</script>
      ...
      <div class="inner">
        ...
        <div class="open-code" role="button" tabindex="0" @click="emit('open-opencode-dialog')"
          ...
          <div class="main">
            ...
            <div class="ball-row">
              <div v-for="(ball, idx) in displayBalls" :key="`ssc-open-${anim.revealToken}-${idx}`" class="ball-warp"
            ...
            <div v-if="openCode.length > 0 && !anim.rolling" :key="`ssc-meta-${anim.revealToken}`" class="open-meta">
      ...
      .open-code {
        ...
        .main {
          ...
          .ball-warp {
            ...
            gap: 2px;
            /* 新一期開出：依序落下彈跳（--i 是第幾顆） */
            animation: ssc-ball-in 0.5s cubic-bezier(0.2, 0.9, 0.3, 1.3) both;
            animation-delay: calc(var(--i, 0) * 0.12s);

            /* 準備開獎／開獎中：持續翻點抖動 */
            &.is-rolling {
              animation: ssc-ball-roll 0.32s linear infinite;
          ...
          .open-meta {
            ...
            animation: ssc-meta-in 0.4s ease both;
