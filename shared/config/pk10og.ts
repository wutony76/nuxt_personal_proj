 ...
 */
export type Pk10OgBetResult = { status: 'win' | 'lose' | 'tie'; odds: number; payout: number }

/**
 * 注碼賠率（含本金）＝ 公平賠率 × rtp
 * @returns 賠率，取到小數 2 位；注碼無法辨識或機率為 0 回 0
 */
export function pk10OgOddsOf(betCode: string, rtp: number = PK10OG_RTP_FALLBACK): number {
  const chance = pk10ChanceOf(betCode)
  if (!chance || !(chance.hit > 0) || !(chance.total > 0)) return 0
  const safeRtp = Number.isFinite(rtp) && rtp > 0 ? rtp : PK10OG_RTP_FALLBACK
 ...
 */
export function judgePk10OgBet(
  betCode: string,
  openCode: Array<string | number>,
  coin = 1,
  lockedOdds = 0
): Pk10OgBetResult | null {
  const hit = pk10IsHit(betCode, openCode)
  if (hit === null) return null

  const odds = lockedOdds > 0 ? Number(lockedOdds) : pk10OgOddsOf(betCode)
