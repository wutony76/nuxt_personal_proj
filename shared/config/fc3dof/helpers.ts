 ...
 *                     注碼由 fc3dComboCodes() 展開，清單裡沒有，改驗 combo 規則。
 *
 * ── 複式的五種展開（對應 fc3d-of.ts）──────────────────────
 ...
 *   input  三星直選單式 → 前端輸入框直接給注碼，不走展開函式（fc3dComboCodes 回空陣列）
 *
 * ⚠️ `三星直選` 是 `三星直選和值` 的前綴，光比對前綴會讓和值注碼誤過直選分頁驗證，
 *    故 fc3dHasBetCode 額外用 _matchesCombo 檢查「解析出的注碼種類」與分頁一致。
 * ⚠️ 本檔 import fc3d-of.ts，因此 fc3d-of.ts 不可反向 import 本檔（會形成循環）。
 */
import C_PLAYS from '#shared/config/fc3dof/plays'
import {
  fc3dOddsOf,
  fc3dKindOf,
  fc3dDirectCombos,
...
import { FC3D_SUM_MAX } from '#shared/config/fc3d'

export type Fc3dOfQuota = {
  /** 單注投注額 */
...
/** 分頁未設定 quota 時的預設值 */
export const FC3D_QUOTA_FALLBACK: Fc3dOfQuota = {
  item: { min: 2, max: 10000 },
  issue: { max: 0 }
}

/** 組選型態（對應 fc3d-of.ts 的 fc3dGroupCombos） */
export type Fc3dOfGroupMode = 'group2' | 'group3' | 'group6'
 ...
 */
export type Fc3dOfCombo = {
  mode: 'direct' | 'group' | 'sides' | 'each' | 'input'
  prefix: string
  section?: Fc3dOfSection
  positions?: number
...
/** 玩法清單（給前端的玩法列用） */
export function fc3dPlays(): ConfigPlay[] {
  return _plays
}

/** 取玩法設定（dingwei / zhixuan / sanxing / budingwei / daxiao） */
export function findFc3dPlay(playKey?: string): ConfigPlay | null {
...
export function findFc3dTab(playKey?: string, tabId?: number | string): ConfigTab | null {
  const play = findFc3dPlay(playKey)
...
/** 取分頁的投注限額 */
export function fc3dQuotaOf(playKey?: string, tabId?: number | string): Fc3dOfQuota {
  const quota = findFc3dTab(playKey, tabId)?.settings?.quota
  return {
    item: {
      min: _num(quota?.item?.min, FC3D_QUOTA_FALLBACK.item.min),
      max: _num(quota?.item?.max, FC3D_QUOTA_FALLBACK.item.max)
    },
    issue: { max: _num(quota?.issue?.max, FC3D_QUOTA_FALLBACK.issue.max) }
  }
}

/** 取分頁設定的回報率 */
export function fc3dRtpOf(playKey?: string, tabId?: number | string): number {
  const rtp = Number(findFc3dTab(playKey, tabId)?.settings?.payout?.rtp)
  return Number.isFinite(rtp) && rtp > 0 ? rtp : FC3D_RTP_FALLBACK
}

/** 取分頁設定的賠率上限（未設定回 0 表示不封頂） */
export function fc3dMaxOddsOf(playKey?: string, tabId?: number | string): number {
  const maxOdds = Number(findFc3dTab(playKey, tabId)?.settings?.payout?.maxOdds)
  return Number.isFinite(maxOdds) && maxOdds > 0 ? maxOdds : 0
}

/** 解析出的注碼種類是否與該分頁的 combo 規則一致（防止前綴巢狀誤判，見檔頭說明） */
function _matchesCombo(combo: Fc3dOfCombo, betCode: string): boolean {
  const kind = fc3dKindOf(betCode)
 ...
 * 複式分頁：注碼由前端展開、清單裡沒有 —— 改驗「前綴符合 + 解析種類符合 + 能被 fc3d-of.ts 判定
 *          （fc3dOddsOf > 0，等同 fc3dChanceOf() !== null 且命中數 > 0）」。
 */
export function fc3dHasBetCode(playKey?: string, tabId?: number | string, betCode?: string | number): boolean {
  ...
  if (combo) {
    ...
    return fc3dOddsOf(code, fc3dRtpOf(playKey, tabId)) > 0
  }
 ...
 * @returns 一注一碼的清單；規則不合或超過 FC3D_MAX_COMBO 上限回空陣列
 */
export function fc3dComboCodes(
  playKey?: string,
  tabId?: number | string,
  picks?: Array<Array<number | string>>
): string[] {
  const combo = fc3dComboOf(playKey, tabId)
  if (!combo) return []
  ...
  if (combo.mode === 'group') {
    ...
    return fc3dGroupCombos(pool, combo.group).map((digits) => `${combo.prefix}${digits.join('')}`)
  }
  ...
  if (combo.mode === 'sides') {
    return fc3dSideCombos(used.map((list) => list.map((s) => String(s))))
      .map((sides) => `${combo.prefix}${sides.join('')}`)
  }
  return fc3dDirectCombos(used).map((digits) => `${combo.prefix}${digits.join('')}`)
}
 ...
 *          （values 含號碼 0~9 或和值 0~27，故上限用 FC3D_SUM_MAX）。
 */
export function fc3dComboGroups(
  playKey?: string,
  tabId?: number | string
    ...
    .map((group, idx) => {
      ...
      return {
        ...
        values: list
          ...
          .filter((v) => Number.isInteger(v) && v >= 0 && v <= FC3D_SUM_MAX),
