 ...
 *
 * ⚠️ PK10 一期開 10 台車，「開獎」欄只畫前三名 —— 10 顆球會把欄寬撐爆，
 *    完整名次看開獎歷史彈窗（那邊是一期一列，畫得下）。
 */
const { userRecord: mxRecord, isCd, fetch: mxFetch, actions: mxActions } = usePk10()
...
const _handlers = {
  ...
  /** 注碼顯示：彩池注單的 bet_code 是車號陣列（["5","3","10"]），組成 05-03-10 才看得懂 */
  codeLabelOf: (betCode: string[]) => {
    const list = Array.isArray(betCode) ? betCode : []
    if (list.length === 0) return '—'
    // 全是純數字 → 前三直選的車號陣列；否則是帶前綴的注碼（冠軍05、和大單…）
    if (list.length > 1 && list.every((code) => /^\d+$/.test(String(code)))) {
      return mxActions.comboLabelOf(list)
    }
...
<template>
  <section class="block-main pk10-report">
    <div class="rp-head">
      ...
      <table class="report-table rp-table" :class="{ 'is-empty': !hasData }">
        <thead>
          <tr>
            ...
            <th>注碼</th>
            <th>金額</th>
            <!-- 官方盤兩套派彩並存：賠率玩法顯示賠率、前三直選顯示命中名次數 -->
            <th>{{ isCd ? '賠率' : '賠率／命中' }}</th>
            <th>結果</th>
            <th>派彩</th>
            <th>開獎</th>
        ...
        <tbody>
          <tr v-for="row in pagedRows" :key="row.orderId" :class="`is-${row.winStatus}`">
            ...
            <!-- odds > 0 代表賠率玩法（下注時鎖了賠率）；否則是前三直選的分層 -->
            <td v-else-if="Number(row.odds) > 0" class="t-num">{{ row.odds }}</td>
            <td v-else class="t-num">{{ row.matchCount }} 個<em v-if="row.tierName">（{{ row.tierName }}）</em></td>
            <td class="t-status">{{ statusText(row.winStatus) }}</td>
            <td class="t-num t-payout">{{ row.winAmount > 0 ? money(row.winAmount) : '—' }}</td>
            <td class="t-open">
              <span v-if="row.openCode?.length" class="open-cars">
                <Car v-for="(code, idx) in row.openCode.slice(0, OPEN_TOP)" :key="idx" :car="code" size="xs" />
                <em v-if="row.openCode.length > OPEN_TOP" class="open-more">…</em>
              </span>
              <em v-else>—</em>
            </td>
          </tr>
...
.pk10-report {
  ...
  .rp-table {
    ...
    /* 投注單號：單號要能整段複製（對帳用），所以顯示完整字串不截字。
       ...
       寬度按最長的單號實測：前三直選全複式一次 720 注，單號長到
       PK10-OF20260817156000726(720/720)（33 字），11px 下需 225px，這裡給 240px 留餘裕。
       ⚠️ 220px 就會折行（實測過）—— 改小之前請先用最長單號量一次。 */
    .th-order,
    .t-order {
      width: 240px;
      font-size: 11px;
...
/* PK10 專屬：開獎欄只畫前三名，多的用「…」表示（完整名次看開獎歷史彈窗） */
.pk10-report .rp-table .t-open .open-more {
  font-style: normal;
  font-weight: 700;
  color: var(--color-red-desc);
}
