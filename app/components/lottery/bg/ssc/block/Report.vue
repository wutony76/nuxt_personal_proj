 ...
 *    不像那邊要截前三名再補一個「…」。
 */
const { userRecord: mxRecord, isCd, fetch: mxFetch, actions: mxActions } = useSsc()
...
const _handlers = {
  ...
  /**
   * 注碼顯示
   * ⚠️ 時時彩的注碼一律是帶前綴的字串（第一球7、總和大、後三組六123、大小單雙後二大單），
...
<template>
  <section class="block-main ssc-report">
    <div class="rp-head">
...
/* 卡片外框由全域 .lottery-ssc .block-main 提供；表格沿用全域 .report-table（同 6hc / k3 / pk10） */
.ssc-report {
  flex: 1 1 auto;
  min-width: 0;
  min-height: 0;
  ...
  .rp-table {
    ...
    /* 投注單號：單號要能整段複製（對帳用），所以顯示完整字串不截字。
       ...
       寬度按最長的單號估：複式一次最多 2000 注（SSC_OG_MAX_COMBO），單號長到
       SSC-OF20260818114000726(2000/2000)（34 字），與 pk10 的 33 字同一級距，
       11px 下需 225px 左右，這裡沿用 240px 留餘裕。
