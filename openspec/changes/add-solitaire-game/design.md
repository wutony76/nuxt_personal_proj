...
- 現有遊戲的計分方向多數是「開放式無上限」，PONG（勝場數）與 MINESWEEPER（5 關封頂總分）是兩個例外。SOLITAIRE 的分數介於兩者之間：事件觸發加分（比照 tetriminos／match3／spaceShooter 的模型）但整體受牌局規模限制、非完全開放，且因為 Stock 循環次數不限，需要額外的防刷分設計（見 Decision 5）。
...
- 不做每日挑戰／成就／排行榜／統計。
- 不限制 Stock 循環次數（採用 MVP 允許無限循環，見 Decision 5 的配套防刷分設計）。
...
5. **計分模型：事件觸發加分＋防刷分限制，Stock 循環次數不限但「翻新牌」只在該牌本局首次翻正面時給分**
   - 理由：合法移動 +5、Foundation +10、完成 Bonus +200 三項都是離散事件加分（比照 tetriminos／match3／spaceShooter 模型）。但使用者規格明確要求 MVP 允許 Stock 無限次重新循環 Waste，若「翻出新牌 +10」不加限制，玩家可以無限循環抽牌刷分而不必真的解牌——這是本次計分設計必須處理的漏洞，已與使用者確認採用「只認該牌這局第一次翻正面」的限制方案。
   - 做法：`SolitaireCoreEngine` 內部維護一個 `Set<cardId>` 記錄「本局曾經翻正面過的牌」，`drawFromStock()`／自動翻牌邏輯只在該牌 ID 第一次加入這個 Set 時才回傳「+10」的分數事件，循環重複抽到同一張牌不再加分。
   - 估算：中高水準對局總分落在 300～700 分，比照多數遊戲預設抓 `coinRate: 0.3`、`coinCapPerRun: 300`、`coinDailyCap: 100000`，`maxReasonableScore(): 3000`（防偽造寬裕上限）。這些是估算值，上線後依實測校準。
...
- [風險] 防刷分的「本局首次翻正面」限制是新設計，尚未經過實測驗證是否還有其他漏洞（例如反覆搬移合法移動洗 +5 分）——因應：合法移動的「+5」本身有意義（確實改變了牌局狀態），比照 `coinCapPerRun` 既有機制夾住單局上限，不需要額外限制移動次數。
...
- Stock 循環的防刷分限制（本局首次翻正面才加分）上線後應實測是否還有其他刷分路徑，尤其是合法移動來回搬動的 +5。
