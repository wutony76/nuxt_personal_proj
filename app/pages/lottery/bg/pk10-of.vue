...
/**
 * 未登入處理：與 pk10-cd 一致 —— 先 await useAuth().init() 確認登入狀態，
...
<template>
  <div class="base lottery-pk10 is-of">
    <div class="bg-fx">
    ...
    <main class="main">
      ...
      <section class="play-warp">
        ...
        <div class="play-tabs">
          ...
          <NuxtLink to="/lottery/bg/pk10-cd" class="mode-link">切換信用玩法 →</NuxtLink>
        </div>
...
.lottery-pk10.is-of {
  ...
  .play-warp {
    ...
    /* 隨機選號：定義在 .play-warp 這一層而不是綁在 .play-tabs 底下 ——
       選擇器不綁父層才不會一搬位置就掉樣式（pk10-cd 同樣處理）。
...
/* PK10-OF 專屬：彩池玩法的小標籤（玩法列與注項盤標題各一處） */
.lottery-pk10.is-of {
  .play-warp .play-tabs .play-tab .play-tag {
    margin-left: 4px;
    border-radius: 3px;
  ...
  .selector-warp .selector .head .head-tag {
    margin-left: 6px;
    border-radius: 3px;
