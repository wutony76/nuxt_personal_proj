...
import { findKl10Tab, kl10ChosenOf, kl10QuotaOf, kl10TabOddsOf } from '#shared/config/kl10cd/helpers'

/**
 * 快樂十分（KL10）前端狀態
 *
 * ── 與 useEggs 的差異：多一套「任選」狀態 ────────────────
 *   來源（bglottery `kl10/`）只有信用模式、沒有官方盤，
 *   所以沒有 mode / setMode、沒有官方盤選號、也沒有共用彩池 —— 這部分與 useEggs 相同。
 *   不同的是「任選」是複式玩法：先選 k 個號碼，再依 C(k, N) 展開成多注
...
const state = reactive({
  /** 當前玩法（zhenghe / longhu / renxuan / liangmian） */
...
const current = reactive({
  runtime: null as Kl10Current | null,
...
const select = reactive({
  items: [] as Kl10SelectItem[],
  pool: [] as Kl10SelectItem[],
  show: true,
  resetToken: 0
})
 ...
 *    塞進同一個 pool 會讓 Board 與送單流程各自長分支，所以分開放。
 */
const renxuan = reactive({
  /** 號碼池 01~20 的選取狀態 */
  pool: KL10_NUMBERS.map((num) => ({ num, label: kl10NumberLabel(num), select: false })),
  /** 單注金額（每個組合各下這個金額） */
  amount: 10 as number
})
...
const userRecord = reactive({
  ...
  betHistory: [] as Kl10UserBetHistory[],
...
const _handlers = {
  ...
  renxuanCodeOf: (nums: number[]): string =>
    `${state.selectTabName}${nums.map((num) => kl10NumberLabel(num)).join(',')}`
}
...
const currentChosen = computed(() => kl10ChosenOf(state.select, state.selectTabId))
...
const renxuanOdds = computed(() => {
  ...
  if (!first) return 0
  return kl10TabOddsOf(state.select, state.selectTabId, _handlers.renxuanCodeOf(first))
})
/** 任選本次投注總額 = 單注 × 注數 */
...
const _actions = {
  ...
  setTab: (tabId: number | string) => {
    const tab = findKl10Tab(state.select, tabId)
    if (!tab) return
  ...
  registerSelectPool: (items: Kl10SelectItem[]) => {
  ...
  randomSelect: (count: number) => {
    ...
    for (let i = shuffled.length - 1; i > 0; i--) {
      ...
      const tmp = shuffled[i] as Kl10SelectItem
      shuffled[i] = shuffled[j] as Kl10SelectItem
      shuffled[j] = tmp
    }
  ...
  sumOf: (openCode: string[]) => {
    const nums = kl10NumbersOf((Array.isArray(openCode) ? openCode : []).map(Number))
...
const fetch = {
  ...
  refreshCurrentInfo: async () => {
    try {
      const result = await api.lottery.currentKl10()
      if (!result) return
  ...
  userRecordAll: async () => {
    ...
    try {
      const res = await api.lottery.userRecordKl10()
  ...
  openCodeHistoryAll: async () => {
    ...
    try {
      const res = await api.lottery.openCodeHistoryKl10()
  ...
  claimOneIssue: async () => {
    ...
    try {
      const res = await api.lottery.claimOneIssueKl10()
      if (res?.ok) {
   ...
   * ⚠️ 賠率不從這裡送（伺端會用 kl10TabOddsOf 依注碼重算後鎖進注單）。
   */
  betsRenxuan: async () => {
    const chosen = currentChosen.value
    if (!chosen) {
    ...
    const playId = String(
      findKl10Tab(state.select, state.selectTabId)?.tabGroup?.[0]?.groupList?.[0]?.playId ?? ''
    )
    return fetch.submit(
      [{
        playKey: state.select,
