...
import {
  ...
  x5JackpotWeightOf,
  findX5Tab
} from '#shared/config/x5cd/helpers'
import {
  X5_SHARED,
  x5AddIssueJackpot,
 ...
 *   抽水一律進 X5_SHARED.pool，兩個盤口的投注共同養同一個彩池。
 *
 * ── 爆池（兩個盤口共吃一池，狀態在 x5Shared.ts）──────────
 *   每注另抽 X5_JACKPOT_SETTINGS.rakeRatio 進爆池，開出「五球全單或全雙」那一期
 *   一次發放給該期有份的注單（依注金 × 看板設定的 weight 分配）。
 *   本 class 只負責：判定自己的注單 → x5SubmitJackpotRows('cd') → 把 source === 'cd'
 *   的分配結果寫回自己的 record；池、滾存與「只結算一次」的保證都在共用層。
 *   ⚠️ 爆池要與 X5_SHARED.pool 分開 —— 那個池是官方盤直選類（階段 2）要吃的，
 *      兩條結算路搶同一個 carry 會互相吃掉對方的滾存。
 *
 * ── 與 SSC-CD 的差異 ────────────────────────────────────
 *   開獎是 1 ~ 11 取 5 個**不重複**號碼（比較接近 pk10 的排列），故 randomOpenCode 走洗牌。
 *   信用盤的注項沒有「和」（五碼不重複，龍虎兩球位不可能相等），判定只有 win／lose ——
 *   `tie` 只在注碼無法辨識時用來退還本金，避免吞掉玩家注金。
 *   其餘（一注一注項、賠率下注時鎖進注單、單期分頁限額）與 SSC-CD 完全一致。
 */
...
type UserStoreLike = { userId?: string; coin?: number; x5Record?: UserRecord }

/** 抽水比例：投注額的固定比例撥入共用彩池（賠率派彩由莊家支付） */
const X5_RAKE_RATIO = 0.02
...
/**
 * 取一注的注碼
 * 11選5 信用盤的注項名稱本身就是注碼（"第一球07"、"總和尾大"、"龍虎12龍"、"全中07"…），
...
export default class X5_CD extends LOTTERY_BASE {
  ...
  constructor() {
    super(LOTTERY['X5-CD'].key, LOTTERY['X5-CD'].id)
    this.issueSettledMap = {}
    // 爆池狀態放共用層（兩個盤口共吃一池），本 class 只負責交件與寫回自己的 record
    x5RegisterJackpotBoard('cd')
    ...
    Object.assign(this.handle, {
      // 11選5 開獎：1 ~ 11 取 5 個不重複號碼
      randomOpenCode: () => x5RandomOpenCode(),
      /**
       * 覆寫期表產生：改由 x5Shared 持有，與 X5-OF（階段 2）共用同一份開獎號
      ...
      /** 產生當日期表（期別／時間沿用 base 的週期常數，開獎號用 11選5 的 5 碼） */
      buildDayRecords: (now: Date): OpenCodeRecord[] => {
      ...
      ensureUserRecord: (user: UserStoreLike) => {
        // 與 6hc（record）、快3、PK10、時時彩、PC蛋蛋分開存，各彩種的注單紀錄互不干擾
        if (!user.x5Record) {
          user.x5Record = { balanceChanges: [], betHistory: [], claimableIssues: [], updatedAt: Date.now() }
        }
        if (!Array.isArray(user.x5Record.balanceChanges)) user.x5Record.balanceChanges = []
        if (!Array.isArray(user.x5Record.betHistory)) user.x5Record.betHistory = []
        if (!Array.isArray(user.x5Record.claimableIssues)) user.x5Record.claimableIssues = []
        user.x5Record.updatedAt = Date.now()
        return user.x5Record
      },
      ...
      validateBetQuota: (input: { issue: string; userId: string; amount: number; groups: Group[] }) => {
        ...
        ;(Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
          ...
          ;(Array.isArray(group?.playList) ? group.playList : []).forEach((play) => {
            ...
            const tabName = findX5Tab(playKey, tabId)?.tabName ?? String(tabId)
            const betCode = _resolveBetCode(play)
            // 注項必須真的存在於該分頁，否則前端亂送也能建單
            if (!betCode || !x5HasBetCode(playKey, tabId, betCode)) {
              this.handle.rejectBet(`${tabName}「${betCode || '(空白)'}」不是有效注項`)
            }
            const quota = x5QuotaOf(playKey, tabId)
        ...
        ;(Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
          ...
          playList.forEach((play, index) => {
            ...
            rows.push({
              ...
              odds: x5TabOddsOf(playKey, tabId, betCode)
            })
          })
        })
        return rows
      ...
      settleIssuePrize: (issue: string, openCode: string[]) => {
        ...
        issueOrders.forEach((row) => {
          ...
          const judged = judgeX5Bet(betCode, codes, coin, lockedOdds, x5RtpOf(playKey, tabId))
          // 無法辨識的注項視為和局退還本金，避免吞掉玩家注金
          const result: X5BetResult = judged?.result ?? 'tie'
        ...
        // ⚠️ 爆池 share 必須在這個迴圈之前併進 payoutByUser，否則玩家領不到爆池加碼
    ...
    Object.assign(this.get, {
      ...
      /**
       * 爆池狀態（兩個盤口共吃的池，與 poolState() 的共用彩池是兩回事）
       * ⚠️ 那邊是官方盤直選類（階段 2）要吃的池，這邊是爆池
       */
      creditJackpot: () => x5JackpotState(this._get.latestIssue()),
      /** 該期開獎的總和（15 ~ 45，供前端顯示與冷熱分析） */
  ...
  playBets(payload: PlayBetsPayload, user: UserStoreLike) {
    ...
    // 抽水入共用彩池（X5-CD 與 X5-OF 共同養同一個池，官方盤直選類在吃）
    x5AddIssuePool(issue, Number((amount * X5_RAKE_RATIO).toFixed(2)))
