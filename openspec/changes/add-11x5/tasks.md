...
## 11. 階段 2：官方盤（2026-08-19 實作）

- [x] 11.1 `shared/config/x5-of.ts`：直選／組選／定位膽／不定位／任選 N 中 M／猜中位／定單雙的機率與判定
- [x] 11.2 `shared/config/x5of/{plays.js,helpers.ts}`：8 分頁、含 `selectarea` 佈局（複式／單式／膽拖三種選號型態）
- [x] 11.3 膽拖注數展開：`C(拖碼數, 目標碼數 − 膽碼數)`（來源 `algorithm.js:251-267`），膽碼上限 `目標碼數 − 1`，膽拖不同號（來源 `select_num_tool.js:69-90`）
- [x] 11.4 `server/services/game/lottery/bg/x5Of.ts` + `server/api/lottery/x5-of/**`（含共用彩池 `pool` 欄位）
- [x] 11.5 `of/base/Board.vue`（含膽拖選號 UI）／`of/block/footer/Auto.vue`／`app/pages/lottery/bg/11x5-of.vue`
- [x] 11.6 `x5Shared.ts` 接上官方盤結算編排（等兩盤口都交件才結算共用彩池）
- [x] 11.7 `app/config/constants.js` 登記 `LOTTERY['X5-OF']`（id 600101 / sort 601）、`useX5.ts` 的 `lotteryMeta` 改回依 `isCd` 分流、大廳移除 `GAME_MODES.X5` 覆寫、`ROUTE_DICT` 補 `X5-OF`、`BgAutoPanel` 補 `X5OfAuto`、`useBgAutoActive` 加 `'x5-of'`
- [x] 11.8 補回階段 1 為了「只有信用盤」而拿掉的共用元件官方盤分支（**這幾支不補，官方盤頁會壞**）：
      `Controls.vue`（投注鈕改讀 `canSubmitOf` / 送 `betsOf()`，否則按不下去、且會送成 CD payload）、
      `CurrItems.vue`（補 `ofRows`：單選／單式逐項、展開型列前 50 注）、
      `Report.vue` 與 `DialogUser.vue`（補回 `tierName` 顯示與「賠率／分層」欄名）、
      `DialogRule.vue`（補官方盤 54 分頁的注項表、四種選號規則說明、彩池分層與官方盤專屬的特別說明）
- [x] 11.9 命名統一：`x5Og*` / `X5_OG_*` 一律改為 `x5Of*` / `X5_OF_*`（不再沿用 sscog 的 og 命名）

## 12. 驗證（階段 2）

- [x] 12.1 窮舉對帳（esbuild + node）：後三直選命中分布 746 / 219 / 24 / 1（合計 990，對照 ssc-of 的 729/243/27/1）；
      三碼直選 990、三碼組選 165、二碼直選 110、二碼組選 55、定位膽 11、一碼不定位 3.667；
      任選一~八 5/11・10/55・10/165・5/330・1/462・6/462・15/330・20/165；
      猜中位 28/63/90/100/90/63/28（合計 462）；定單雙 1/30/150/200/75/6（合計 462）
- [x] 12.2 `x5OfIsHit()` 22 個情境：直選位置錯不中／組選不計順序／組選注碼未遞增回 null／
      任選 N ≤ 5 與 N > 5 的判定方向相反／不定位只看該區段／猜中位取排序後中位數／
      定單雙數單數個數／開獎有重複回 null
- [x] 12.3 展開對帳：直選三位全選 = 990（**不是 1331**，已濾重複號碼）／組選 C(n,k)／
      任選五中五全選 = 462／膽拖 = C(拖, N−膽)／膽碼數 ≥ N 回 0 注／膽拖同號自動濾除
- [x] 12.4 單式列舉：990 / 165 / 110 / 55 / 11 / 462 / 165 筆，注碼不重複且逐筆通得過 `x5OfHasBetCode()`
- [x] 12.5 config 層 54 分頁全數對帳：展開型 30 / 單式 18 / 單選 6，彩池分頁 2；
      101 個單選注項的賠率與 weight 逐項正確；彩池分頁 odds 回 0 但注碼照常驗證、weight 3
- [x] 12.6 `npm run build` exit code 0（Σ 6.72 MB；`11x5-of` 頁面 chunk 與 `api/lottery/x5-of` 五支路由都有進 build）
- [x] 12.7 服務啟動與共用層：dev log 出現 `TTT---RUN.X5.官方`；
      `x5-cd/current` 與 `x5-of/current` 回**相同**期別（20260819104）、開獎號（01 05 11 06 03）與彩池（base 295504）
- [x] 12.8 官方盤下注（curl）：一次送 5 種分頁 —— 後三直選複式（彩池，odds 鎖 0）／定位膽 10.55／
      任選三中三 15.84／猜中位06 4.43／任選二中二單式 5.27，全部建單成功、餘額正確扣款
- [x] 12.9 封盤閘門：已封盤時送單被擋（`目前為「已封盤」，不受理投注`），且擋在驗證與扣款之前
- [x] 12.10 官方盤結算**雙路徑**（第 20260819125 期，開獎 06 07 05 09 10、後三 = 05 09 10）：
      **彩池路** —— 後三直選複式三位全選下 990 注（這樣三個分層必然都有中獎者），
      實際結算命中分布 **746 / 219 / 24 / 1**，與 `x5OfMatchCounts()` 窮舉完全一致；
      頭獎 1 注得 134,546.46（= 可發放池 192,209.23 × 70%）、二獎 24 注共 38,441.76（× 20%）、
      三獎 219 注共 876（固定 2 倍 × 注金 2）；三層都有人中 → `carry` 正確為 0、該期 `issuePool` 歸零
      **賠率路** —— 定位膽第一球全選 11 注中 1（第一球06 × 10.55 = 105.5）、
      猜中位全選 7 注中 1（猜中位07 × 4.92）、任選一中一全選 11 注中 5（各 × 2.11），
      `tierName` 正確寫入「中獎」
      **兩條抽水線** —— 共用彩池 +1,362（總額 2,270 × 0.6）、爆池 +22.70（× 0.01），互不相吃
      **領獎** —— 可領 174,124.42 = 彩池 173,864.22 + 賠率 260.2 + 爆池 0，領獎後餘額正確到帳
      （該期爆池未觸發：06 07 05 09 10 單雙混合，`carry` 累加到 27.74）
- [x] 12.11 注碼拒單 10 項全部 400 且**餘額未變**：直選含重複號碼（`後三直選010101`）／
      組選注碼未遞增（`後三組選030201`）／注碼放錯分頁（前三直選送後三直選、任選三送任選二單式）／
      號碼超界（12）／猜中位 02（不可能是中位數）／定單雙不存在的結果／完全無法辨識的注碼／
      單注低於下限 2／單注超過上限 10,000
- [x] 12.12 Playwright 實測（無 console pageerror）：
      大廳 11選5 **兩張卡**（X5-OF / X5-CD）；官方盤頁 8 個玩法列與各玩法分頁數正確；
      **展開型** 後三直選複式 3 格 × 11 號、彩池分層說明正確、各格選 2 個 → 展開 8 注；
      **膽拖** 膽 01 + 拖 02/03/04 → 3 注（C(3,2)）、賠率 15.84；把 02 點成膽碼後
      **02 自動從拖碼移除**（互斥生效）且膽碼上限 2 生效 → 展開 2 注；
      **單式** 任選二中二列出 55 注可點選、點 2 注後當前注項卡與總注額同步；
      **單選** 定位膽 5 群組 × 11 = 55 格、趣味玩法定單雙 6 格
      ⚠️ 兩個非問題：定位膽只有 1 個分頁所以分頁列不渲染（與信用盤同邏輯）；
         單式 55 注 ≤ 120/頁所以分頁器不出現
