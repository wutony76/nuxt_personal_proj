...
import History from '~/components/lottery/bg/eggs/block/History.vue'
import Road from '~/components/lottery/bg/eggs/block/Road.vue'
import DialogUser from '~/components/lottery/bg/eggs/block/DialogUser.vue'
import DialogOpenCode from '~/components/lottery/bg/eggs/block/DialogOpenCode.vue'
import DialogRule from '~/components/lottery/bg/eggs/block/DialogRule.vue'
 ...
 * PC蛋蛋（EGGS）信用玩法
 *
 * 注項一律讀 shared/config/eggscd 的設定檔（與 k3-cd 同一套結構），
 * 賠率由 helpers 依該分頁 rtp 即時推算，畫面顯示與伺端鎖進注單的值一致。
 * ⚠️ 與 k3-cd 最大的差異：PC蛋蛋只有信用盤，沒有官方盤／共用彩池，
 *    所以版面拿掉了獎金框、盤口切換連結這兩塊 k3 專屬的區塊；路珠走勢因為只是
 *    開獎歷史的統計呈現、不涉及玩法規則，仍照 K3 版面保留。
 */
const {
  state: mxState,
  wallet: mxWallet,
...
<template>
  <div class="base lottery-eggs">
    ...
    <main class="main">
      <EggsHeader @open-opencode-dialog="dialogClick.openOpenCode()" />
...
.lottery-eggs {
  ...
  /* 背景上升光點（同 k3-cd 的 .bg-fx，keyframes 另立 eggs- 前綴避免依賴他頁） */
  .bg-fx {
    position: fixed;
    inset: 0;
    pointer-events: none;
    ...
    .orb {
      ...
      animation: eggs-orb-rise calc(7s + var(--i) * 0.6s) ease-in infinite;
  ...
  .info-warp {
    ...
    gap: 0.75rem;
    align-items: stretch;
    animation: eggs-sec-in 0.55s ease both;
    ...
    .info-side {
      ...
      .user-warp {
        ...
        cursor: pointer;
        animation: eggs-card-glow 3.5s ease-in-out infinite;
