...
const rows = computed(() => mxHistory.list.slice(0, HIST_ROWS).map((item) => {
  const digits = pl3DigitsOf(item.openCode)
  const sum = digits ? pl3SumOf(digits) : 0
  const isTriple = digits ? pl3IsTriple(digits) : false
  return {
    issue: String(item.issue),
    openCode: item.openCode,
    sum,
    isBig: sum > PL3_SUM_BIG_LINE,
    isOdd: sum % 2 === 1,
    isTriple
  }
}))
...
<template>
  <div class="block-main pl3-history">
    <div class="hist-head">
