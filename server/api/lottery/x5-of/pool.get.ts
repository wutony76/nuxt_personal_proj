...
/**
 * 11選5：共用彩池狀態（官方盤吃池分頁分層吃這個池，X5-CD 與 X5-OF 共用同一份）
 *
 * ⚠️ 與 jackpot.get.ts 是兩個完全獨立的池——這支是官方盤分層派彩用的共用彩池，
 *    jackpot.get.ts 是爆池（兩個盤口共吃、開出爆池條件才發放），兩者互不影響。
 * ⚠️ 這份資料原本只內嵌在 /x5-of/current 的 `pool` 欄位裡，這裡另開一支給不需要
...
export default defineEventHandler(() => {
  const game = Storage.games[LOTTERY['X5-OF'].key] as X5OfService | undefined
