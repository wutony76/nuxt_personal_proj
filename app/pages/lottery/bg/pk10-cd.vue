...
import Pk10Board from '~/components/lottery/bg/pk10/cd/base/Board.vue'
...
import History from '~/components/lottery/bg/pk10/block/History.vue'
import Road from '~/components/lottery/bg/pk10/block/Road.vue'
import DialogUser from '~/components/lottery/bg/pk10/block/DialogUser.vue'
import DialogOpenCode from '~/components/lottery/bg/pk10/block/DialogOpenCode.vue'
import DialogRule from '~/components/lottery/bg/pk10/block/DialogRule.vue'
 ...
 * PK10 信用玩法（PK10-CD）
 *
 * 版面與 k3-cd 完全同一套（頁首／使用者卡＋開獎歷史＋路珠／投注區／注單）。
...
/** 下方的自動下注／CHAT 面板由 app.vue 的 BgAutoPanel 統一渲染（同 k3 / 6hc-of） */
...
/**
 * 未登入處理：與 k3 / 6hc 一致 —— 先 await useAuth().init() 確認登入狀態，
...
<template>
  <div class="base lottery-pk10 is-cd">
    <!-- 背景光點（同 k3 / 6hc-cd） -->
    <div class="bg-fx">
    ...
    <main class="main">
      <Pk10Header @open-opencode-dialog="dialogClick.openOpenCode()" />
      ...
      <section class="play-warp">
        ...
        <div class="play-tabs">
          ...
          <NuxtLink to="/lottery/bg/pk10-of" class="mode-link">切換官方玩法 →</NuxtLink>
        </div>
...
/* ⚠️ 本頁 <style> 沒有 scoped，選擇器多帶一個 .is-cd 用來隔開另一個盤口 ——
   pk10-cd 與 pk10-of 共用 .lottery-pk10 根 class，NuxtLink 會預抓對面那頁的 CSS 並注入，
...
.lottery-pk10.is-cd {
  ...
  /* 背景上升光點（同 6hc-cd 的 .bg-fx，keyframes 另立 pk10- 前綴避免依賴他頁） */
  .bg-fx {
    position: fixed;
    inset: 0;
    pointer-events: none;
    ...
    .orb {
      ...
      animation: pk10-orb-rise calc(7s + var(--i) * 0.6s) ease-in infinite;
  ...
  .info-warp {
    ...
    gap: 0.75rem;
    align-items: stretch;
    animation: pk10-sec-in 0.55s ease both;
    ...
    .info-side {
      ...
      /* 樣式參照 6hc-of 的 .user-warp（app/pages/lottery/bg/6hc-of.vue）：
         ...
         淡紅底 + 呼吸光暈。差異：PK10 側欄寬 240px（6hc-of 是 22%），
         且數值仍用 <b> 標粗、F幣餘額維持綠色。 */
      .user-warp {
        ...
        cursor: pointer;
        animation: pk10-card-glow 3.5s ease-in-out infinite;
  ...
  .play-warp {
    ...
    /* 注項面板：外層容器與 6hc-of 的 .selector-warp 一致
       ...
       裡層 .selector（帶標題列的白框）是 PK10 自己的，沿用 6hc-cd 的做法。 */
    .selector-warp {
      margin-top: 12px;
      display: flex;
      gap: 12px;
