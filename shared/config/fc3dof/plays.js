 ...
 * 福彩3D官方盤看板設定總表
 *
 * 玩法、分頁與注項順序照 bglottery `fc3d/config_ssc.js`（lotteryId 1801，只有官方盤），
 * 分頁 id 直接沿用來源的 playId 方便對帳，文案改繁體。5 個玩法分頁：
 *   18110 定位膽      ← 百/十/個位各自 0~9 單選（單選分頁）
 *   18111 直選組選    ← 前二/後二 直選（位置對應）／組選（2 碼不分順序）
 *   18112 三星        ← 直選複式/單式/和值、組三/組六/組選和值
 *   18113 不定位      ← 一碼不定位／二碼不定位
 *   18114 大小單雙    ← 前二/後二 各位置的 大/小/單/雙
 *
 * ── 分頁型態（對應 fc3dof/helpers.ts 的展開與驗證）──────────
 *   單選分頁（定位膽）—— groupList 就是注項清單，注碼＝name（百位0…）
 *   複式分頁（其餘）—— groupList 只是「該位置／該組可選的號碼或面」，
 *     注碼由 fc3dComboCodes() 依 combo.mode 展開，清單裡找不到，驗證改走 combo 規則：
 ...
 *
 * ⚠️ 賠率一律由 fc3d-of.ts 依「公平賠率（母數÷命中數）× rtp」推算，本檔 odds／註解只是快照。
 ...
 * ⚠️ 來源 `181121113`（三星組選和值·組三）／`181121114`（·組六）在 config_ssc.js 是**空物件
 *    `{}`**（無 selectarea/alias、UI 打不到），已於提案階段與使用者確認**跳過不實作**，
 *    只做合併版 `181121112` 三星組選和值（見下方 sanxing 分頁）。
 */
    ...
    list: [
      {
        ...
        tabGroup: [
          { groupName: '百位', pos: 0, weight: 2, columns: 5, groupList: _balls(181101010, 0, '百位') },
          { groupName: '十位', pos: 1, weight: 2, columns: 5, groupList: _balls(181101010, 1, '十位') },
          { groupName: '個位', pos: 2, weight: 2, columns: 5, groupList: _balls(181101010, 2, '個位') }
        ]
      }
    ]
  },
    ...
    list: [
      {
        ...
        tabGroup: [
          { groupName: '百位', pos: 0, weight: 2, columns: 5, groupList: _digits(181111010, 0) },
          { groupName: '十位', pos: 1, weight: 2, columns: 5, groupList: _digits(181111010, 1) }
        ]
      },
      {
        tabId: 181111011,
        ...
        tabGroup: [
          { groupName: '十位', pos: 0, weight: 2, columns: 5, groupList: _digits(181111011, 0) },
          { groupName: '個位', pos: 1, weight: 2, columns: 5, groupList: _digits(181111011, 1) }
        ]
      },
      {
        tabId: 181111110,
        ...
        tabGroup: [
          { groupName: '組選', pos: 0, weight: 2, columns: 5, groupList: _digits(181111110, 0) }
        ]
      },
      {
        tabId: 181111111,
        ...
        tabGroup: [
          { groupName: '組選', pos: 0, weight: 2, columns: 5, groupList: _digits(181111111, 0) }
        ]
      }
    ]
  },
    ...
    list: [
      {
        ...
        tabGroup: [
          ...
          { groupName: '個位', pos: 2, weight: 2, columns: 5, groupList: _digits(181121010, 2) }
        ]
      },
      {
        tabId: 181121011,
      ...
      },
      {
        ...
        tabGroup: [
          { groupName: '和值', pos: 0, weight: 2, columns: 7, groupList: _sums(181121012, 0, 27) }
        ]
      },
      {
        tabId: 181121110,
        ...
        tabGroup: [
          { groupName: '組三', pos: 0, weight: 2, columns: 5, groupList: _digits(181121110, 0) }
        ]
      },
      {
        tabId: 181121111,
        ...
        tabGroup: [
          { groupName: '組六', pos: 0, weight: 2, columns: 5, groupList: _digits(181121111, 0) }
        ]
      },
      {
        tabId: 181121112,
        ...
        // ⚠️ 來源 181121113（組三和值）／181121114（組六和值）為空物件 {}，已確認跳過，只做本合併版
        combo: { mode: 'each', minPick: 1, prefix: '三星組選和值' },
        tabGroup: [
          { groupName: '和值', pos: 0, weight: 2, columns: 7, groupList: _sums(181121112, 1, 26) }
        ]
      }
    ]
  },
    ...
    list: [
      {
        ...
        tabGroup: [
          { groupName: '不定位', pos: 0, weight: 2, columns: 5, groupList: _digits(181131010, 0) }
        ]
      },
      {
        tabId: 181131011,
        ...
        tabGroup: [
          { groupName: '不定位', pos: 0, weight: 2, columns: 5, groupList: _digits(181131011, 0) }
        ]
      }
    ]
  },
    ...
    list: [
      {
        ...
        tabGroup: [
          { groupName: '百位', pos: 0, weight: 2, columns: 4, groupList: _sides(181141010, 0) },
          { groupName: '十位', pos: 1, weight: 2, columns: 4, groupList: _sides(181141010, 1) }
        ]
      },
      {
        tabId: 181141011,
        ...
        tabGroup: [
          { groupName: '十位', pos: 0, weight: 2, columns: 4, groupList: _sides(181141011, 0) },
          { groupName: '個位', pos: 1, weight: 2, columns: 4, groupList: _sides(181141011, 1) }
        ]
      }
    ]
  }
