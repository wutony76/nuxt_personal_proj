...
遊戲中心目前十款遊戲涵蓋「單次連續衝分」「多局賽制（PONG）」「關卡制解謎（MINESWEEPER）」，但尚未有「迷宮吃豆＋敵人追逐」這個經典類型。新增 PAC-MAN 補齊這個空缺：固定迷宮＋方向鍵移動＋4 隻鬼魂即時追逐，並首度引入「AI 難度依關卡數分層遞增」的設計——鬼魂行為從簡化貪心，到四種經典性格分工，再到 scatter/chase 交替，讓遊戲難度曲線由簡入繁，同時把「高擬真」AI 複雜度封頂在特定關卡之後，避免開發與測試範圍隨關卡數無限擴大。

## What Changes

- 新增遊戲頁面 `app/pages/game/pac-man.vue`（**PAC-MAN**）：
  - 固定一份自製迷宮（19×21 格，柱狀網格生成法保證通道全連通），含左右穿隧通道；4 個大力丸位於四個角落，其餘通道格皆為豆子。
