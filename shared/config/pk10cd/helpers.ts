/**
 * PK10 信用盤看板設定的讀取層（結構與 shared/config/k3cd/helpers.ts 一致）
 *
 * 賠率與限額都以「分頁（tabId）設定」為主，取不到才退回全域預設；
 * 前端顯示 / clamp、伺端驗證 / 派彩全部走這裡，避免各自解析 config。
 *
 * ⚠️ 本檔 import pk10-cd.ts，因此 pk10-cd.ts 不可反向 import 本檔（會形成循環）。
 *    pk10-cd.ts 的判定需要設定值時一律由呼叫端傳入。
 */
import C_PLAYS from '#shared/config/pk10cd/plays'
import { pk10OddsOf, PK10_RTP_FALLBACK } from '#shared/config/pk10-cd'

export type Pk10Quota = {
  /** 單注投注額 */
...
/** 分頁未設定 quota 時的預設值 */
export const PK10_QUOTA_FALLBACK: Pk10Quota = {
...
type ConfigOption = {
  ...
  odds?: number
  weight?: number
  /** 該注項要畫的車號（定位膽／兩面）；判定不讀這裡，一律看 name */
  car?: number
  /** 該注項屬於第幾個名次（定位膽）；同樣只供畫面用 */
  rank?: number
  /** 冠亞組合要畫的兩個車號；同樣只供畫面用 */
  pair?: number[]
}
...
/** 玩法清單（給前端的玩法列用） */
export function pk10Plays(): ConfigPlay[] {
  return _plays
}

/** 取玩法設定（guanya / mingci / longhu） */
export function findPk10Play(playKey?: string): ConfigPlay | null {
...
export function findPk10Tab(playKey?: string, tabId?: number | string): ConfigTab | null {
  const play = findPk10Play(playKey)
...
function _findTabItem(
  ...
  if (!code) return null
  const groups = findPk10Tab(playKey, tabId)?.tabGroup
...
/** 取分頁的投注限額 */
export function pk10QuotaOf(playKey?: string, tabId?: number | string): Pk10Quota {
  const quota = findPk10Tab(playKey, tabId)?.settings?.quota
  return {
    item: {
      min: _num(quota?.item?.min, PK10_QUOTA_FALLBACK.item.min),
      max: _num(quota?.item?.max, PK10_QUOTA_FALLBACK.item.max)
    },
    issue: { max: _num(quota?.issue?.max, PK10_QUOTA_FALLBACK.issue.max) }
  }
}

/** 取分頁設定的回報率 */
export function pk10RtpOf(playKey?: string, tabId?: number | string): number {
  const rtp = Number(findPk10Tab(playKey, tabId)?.settings?.payout?.rtp)
  return Number.isFinite(rtp) && rtp > 0 ? rtp : PK10_RTP_FALLBACK
}

/** 取分頁設定的賠率上限（未設定回 0 表示不封頂） */
export function pk10MaxOddsOf(playKey?: string, tabId?: number | string): number {
  const maxOdds = Number(findPk10Tab(playKey, tabId)?.settings?.payout?.maxOdds)
 ...
 * 取注項賠率（含本金）
 *
 * 一律由 pk10-cd.ts 依「公平賠率 × 該分頁 rtp」推算，而不是讀 config 的 odds ——
 * config 的 odds 只是產生時的快照，改 rtp 就會與實際不符。
 * 若分頁設了 maxOdds 則夾到上限（該注項的實際 RTP 會低於 rtp）。
 * @returns 賠率；注項不在該分頁或無法辨識回 0
 */
export function pk10TabOddsOf(playKey?: string, tabId?: number | string, betCode?: string | number): number {
  const found = _findTabItem(playKey, tabId, betCode)
  if (!found) return 0
  const name = String(found.item.name ?? '')
  const odds = pk10OddsOf(name, pk10RtpOf(playKey, tabId))
  if (!(odds > 0)) return 0
  const maxOdds = pk10MaxOddsOf(playKey, tabId)
 ...
 */
export function pk10JackpotWeightOf(playKey?: string, tabId?: number | string, betCode?: string | number): number {
  const found = _findTabItem(playKey, tabId, betCode)
  if (!found) return 0
...
/** 該注項是否存在於指定分頁（伺端驗證用） */
export function pk10HasBetCode(playKey?: string, tabId?: number | string, betCode?: string | number): boolean {
