...
import { FC3D_SUM_BIG_LINE, fc3dDigitsOf, fc3dSumOf, fc3dIsTriple } from '#shared/config/fc3d'
import { useFc3d } from '~/composables/useFc3d'

/**
 * 開獎歷史彈窗：可搜期號、翻轉排序，並標出和值／大小／單雙／豹子
 * 該期沒有下注的話整列反灰，一眼看得出哪幾期有下注（同 eggs 的 DialogOpenCode.vue）
 */
const props = defineProps<{ visible: boolean }>()
const emit = defineEmits<{ close: [] }>()

const { openCodeHistory: mxHistory, userRecord: mxRecord } = useFc3d()
...
const isTripleOf = (openCode: string[]) => {
  const digits = fc3dDigitsOf(openCode)
  return digits ? fc3dIsTriple(digits) : false
}
</script>
...
<template>
  <DialogShell :visible="props.visible" title="開獎歷史" @close="emit('close')">
    <div class="doc-bar">
      ...
      <span class="doc-note">※ 福彩3D只有官方玩法</span>
    </div>
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="item in rows" :key="item.issue" :class="{ 'is-no-bet': !hasBet(item.issue) }">
            ...
            <td>{{ sumOf(item.openCode) > FC3D_SUM_BIG_LINE ? '大' : '小' }}</td>
            <td>{{ sumOf(item.openCode) % 2 === 1 ? '單' : '雙' }}</td>
            <td>
              <span v-if="isTripleOf(item.openCode)" class="tag is-triple">豹子</span>
