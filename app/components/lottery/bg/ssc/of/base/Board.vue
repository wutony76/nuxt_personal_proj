 ...
 * 時時彩官方盤投注看板
 *
 * 一個元件要應付 config 裡兩種分頁型態（照 pcv2 conf_sc_og.js 的結構）：
 *   單選分頁（定位膽）—— 與信用盤同款的「號碼｜金額」表格，逐項填金額
 *   複式分頁（其餘 10 個）—— 每個位置／每組一列可選值（號碼或面），多選；
 *                          注數＝各列選數相乘（組選則是組合數）
 *
 * ── 為什麼複式要另一種版面 ──────────────────────────────
...
<template>
  <div class="ssc-of-board">
      ...
      <div v-for="group in ogComboGroups" :key="`ssc-of-pos-${group.pos}`" class="pick-row">
        <div class="pick-head">
          ...
          <button type="button" class="pick-all" @click="click.pickAll(group.pos)">
            {{ pickedCountOf(group.pos) === (group.digits.length || group.sides.length) ? '清空' : '全選' }}
          </button>
        </div>
        <!-- 位置直選／組選：號碼球 -->
        <div v-if="group.digits.length > 0" class="pick-values">
          <button v-for="digit in group.digits" :key="`pos-${group.pos}-digit-${digit}`" type="button"
        ...
        <!-- 大小單雙：面別膠囊 -->
        <div v-else class="pick-values is-side">
    ...
    <template v-else>
      ...
      <div v-for="group in singleGroups" :key="`ssc-of-group-${group.groupName}`" class="play-group">
