 ...
 * 合肖看板
 *
 * 結構與連碼同類：注項池由玩家自選（12 生肖選 n 個），選取後展開成 C(已選, pick) 組合，
 * 每個組合才是一注 —— 與 PlayBoard（特碼／正碼…）「一個注項 = 一注」不同，故不能共用。
 *
 * 與連碼的差別：
 *   1. 選取的是「生肖」而非「號碼」，不需色波標色，改用文字膠囊。
 *   2. 賠率不是固定檔次表，取決於「所選的那幾個生肖」（見 creditHexiaoOddsOf），
...
<template>
  <div class="main-play-base hexiao">
    ...
    <div class="play-group">
      ...
      <table class="play-table">
        <tbody>
          <tr v-for="(row, rowIndex) in nameRows" :key="`hexiao-row-${rowIndex}`">
            <td v-for="(item, colIndex) in row" :key="`hexiao-cell-${rowIndex}-${colIndex}`" class="td-code"
