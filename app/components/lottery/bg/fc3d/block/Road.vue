...
import { FC3D_SUM_BIG_LINE, fc3dDigitsOf, fc3dSumOf } from '#shared/config/fc3d'
import { useFc3d } from '~/composables/useFc3d'

/**
 * 福彩3D路珠（版面／邏輯比照 `eggs/block/Road.vue`）
 * 和值 0~27、大小分界 >13 為大（純顯示慣例，見 FC3D_SUM_BIG_LINE），沒有和局的概念。
 */
...
const _handlers = {
  outcomeOf: (openCode: string[], dimension: DimensionKey): Outcome | null => {
    ...
    const sum = fc3dSumOf(digits)
    if (dimension === 'size') return sum > FC3D_SUM_BIG_LINE ? 'big' : 'small'
...
<template>
  <section class="block-main fc3d-road">
    <div class="road-head">
