...
- 新增遊戲頁面 `app/pages/game/solitaire.vue`（**SOLITAIRE**）：
  ...
  - **Stock 循環**：Stock 抽完後，Waste 可重新洗回 Stock 繼續抽，**次數不限**（翻牌本身不計分，見下方計分說明）。
  ...
  - **計分**：翻牌（自動翻牌／Stock 抽牌）不計分，只有實際接上合法序列才算分——合法移動到 Tableau +5、移到 Foundation +10、完成遊戲 Bonus +200（見 design.md Decision 5）
...
- `game-history`：既有的遊戲紀錄／coin 轉換能力擴充支援 `solitaire` 這個新的 `gameKey`；`score` 語意為「合法移動／上疊 Foundation 的事件加分＋完成獎勵」（翻牌本身不計分），`meta` 欄位額外記錄 `moves`／`elapsedSeconds`，是本次唯一同時記錄 moves 與時間的遊戲。
