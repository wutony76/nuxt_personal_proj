...
/**
 * 11選5 官方盤自動下注（版面與信用盤那支 footer/Auto.vue 同一套）
 *
 * 由 app.vue 的 BgAutoPanel 統一渲染，頁面進入時以 useBgAutoActive 啟用。
 * 每期開盤時從當前分頁的注項池隨機取 N 個各下 M 元；以期數為單位記錄，避免同期重複觸發。
 *
 * ⚠️ 與信用盤那支是**兩個獨立元件**（cd/ 與 of/ 各一份），不可共用一個元件用 isCd 分流 ——
 *    BgAutoPanel 的 v-if 鏈若指向同一個元件，Vue 會就地 patch 保留 instance，
 *    切盤口時 enabled 會殘留、對另一個盤口偷偷下注
 *    （openspec/project.md 已記錄這個踩雷點，6hc / k3 / ssc 都是各自一個元件）。
 * ⚠️ 走 fetch.autoBetsOf（直接組 payload），不動使用者手動選的注碼；
 *    但送單成功後會與手動下注一樣清空選取。
 */
const {
  state: mxState, current: mxCurrent, of: mxOf, wallet: mxWallet,
  ofQuota: mxQuota, ofPlayList, ofAutoMaxCount, fetch: mxFetch
} = useX5()

const money = (value: number) => Number(value ?? 0).toLocaleString('zh-TW')

/** 每注金額限額依當前分頁 settings.quota，超限伺端會整筆拒單 */
 ...
 * 注數上限
 *   單選／單式分頁 → 該分頁的注碼數
 *   展開型分頁     → 全選展開的注數（已夾到 X5_OF_MAX_COMBO）
 * ⚠️ 不能用「目前選了幾注」當上限 —— 自動下注根本還沒選號，會一路算成 0
 */
...
const playInfo = computed(() => {
  ...
  return `${play} / ${mxOf.tabName || '-'}：隨機 ${state.betCount} 注（上限 ${maxCount.value} 注）`
})
