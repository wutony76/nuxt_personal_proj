...
/**
 * 11選5 信用盤自動下注（版面與 ssc / 6hc-cd 的 footer/Auto.vue 同一套）
 *
 * 由 app.vue 的 BgAutoPanel 統一渲染，頁面進入時以 useBgAutoActive 啟用。
 * 每期開盤時從當前分頁的注項池隨機取 N 個各下 M 元；以期數為單位記錄，避免同期重複觸發。
 *
 * ⚠️ 放在 cd/ 底下、階段 2 的官方盤要**另外**一個 of/block/footer/Auto.vue，
 *    不可共用一個元件用 isCd 分流 —— BgAutoPanel 的 v-if 鏈若指向同一個元件，
 *    Vue 會就地 patch 保留 instance，切盤口時 enabled 會殘留、對另一個盤口偷偷下注
