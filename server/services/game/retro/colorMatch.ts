...
export default class RetroColorMatchClass extends RETRO_GAME_BASE {
  constructor() {
    super({
      key: 'colorMatch',
      name: 'COLOR MATCH',
      // 生存模式計分（每次配對得 20 分 × 序列長度，milestone 時 double；時間只靠配對成功累加，
      // 沒有固定回合長度，見 colorMatchEngine.ts 的 SCORE_PER_SEQUENCE_STEP／MILESTONE_COMBO_STEP），
      // 無法像固定倒數題目算出精確數學上限，取「一場優秀表現 ≈ 100~150 coin」的既有目標 → coinRate 0.1
      // （SCORE_PER_SEQUENCE_STEP 從 100 調降到 20 後分數整體縮小 5 倍，coinRate 同步放大 5 倍）。
      coinRate: 0.1,
      coinCapPerRun: 150,
      coinDailyCap: 100000
    })
