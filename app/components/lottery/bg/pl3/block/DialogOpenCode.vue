...
import { PL3_SUM_BIG_LINE, pl3DigitsOf, pl3SumOf, pl3IsTriple } from '#shared/config/pl3'
...
const { openCodeHistory: mxHistory, userRecord: mxRecord } = usePl3()
...
const isTripleOf = (openCode: string[]) => {
  const digits = pl3DigitsOf(openCode)
  return digits ? pl3IsTriple(digits) : false
}
</script>
...
<template>
  <DialogShell :visible="props.visible" title="開獎歷史" @close="emit('close')">
    <div class="doc-bar">
      ...
      <span class="doc-note">※ 排列3只有官方玩法</span>
    </div>
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="item in rows" :key="item.issue" :class="{ 'is-no-bet': !hasBet(item.issue) }">
            ...
            <td>{{ sumOf(item.openCode) > PL3_SUM_BIG_LINE ? '大' : '小' }}</td>
