...
import { SSC_BIG_LINE, SSC_SUM_BIG_LINE, sscDigitsOf, sscDragonOf } from '#shared/config/ssc'
 ...
 *
 * ⚠️ 與 pk10 最大的不同：時時彩的號碼**可以重複**，所以龍虎**會出現和局** ——
 *    龍虎那一軸有三種結果（比較像快3 的圍骰要標「和」），不能照抄 pk10 的兩種。
 */
const { openCodeHistory: mxHistory, actions: mxActions } = useSsc()

/** 走勢的四種角度（總和兩面 + 第一球兩面 + 一五球龍虎） */
const DIMENSIONS = [
  { key: 'sumSize', name: '總和大小' },
  { key: 'sumParity', name: '總和單雙' },
  { key: 'ballSize', name: '第一球大小' },
  { key: 'dragon', name: '一五球龍虎' }
] as const
type DimensionKey = (typeof DIMENSIONS)[number]['key']
type Outcome = 'big' | 'small' | 'odd' | 'even' | 'dragon' | 'tiger' | 'tie'

const LABELS: Record<Outcome, string> = {
  big: '大', small: '小', odd: '單', even: '雙', dragon: '龍', tiger: '虎', tie: '和'
}
/** 大／單／龍走紅、小／雙／虎走藍、和走綠（與 Header、開獎歷史同一套配色） */
const TONES: Record<Outcome, string> = {
  big: 'is-red', small: 'is-blue', odd: 'is-red', even: 'is-blue',
  dragon: 'is-red', tiger: 'is-blue', tie: 'is-green'
}
...
const _handlers = {
  outcomeOf: (openCode: string[], dimension: DimensionKey): Outcome | null => {
    const digits = sscDigitsOf(Array.isArray(openCode) ? openCode : [])
    if (!digits) return null
    if (dimension === 'sumSize' || dimension === 'sumParity') {
      /*
       * ⚠️ 不能像 pk10 那樣用 `sum > 0` 當「算得出來」的判斷 ——
       *    時時彩開出 00000 的總和就是 0，那是合法結果，會被整期吃掉。
       *    格式合不合法已經由 sscDigitsOf 判掉了。
       */
      const sum = mxActions.sumOf(openCode)
      return dimension === 'sumSize'
        ? (sum >= SSC_SUM_BIG_LINE ? 'big' : 'small')
        : (sum % 2 === 1 ? 'odd' : 'even')
    }
    if (dimension === 'ballSize') return Number(digits[0]) >= SSC_BIG_LINE ? 'big' : 'small'
    // 一五球龍虎：第一球對第五球，號碼可能相同 → 三種結果
    const result = sscDragonOf(digits, 0, 4)
    if (!result) return null
    return result === '龍' ? 'dragon' : result === '虎' ? 'tiger' : 'tie'
  },
...
<template>
  <section class="block-main ssc-road">
    <div class="road-head">
    ...
    <p class="road-note">※ 時時彩號碼可重複，龍虎會出現「和」；往左拉可看更早的走勢</p>
  </section>
</template>

<style scoped lang="scss">
/* 卡片外框由全域 .lottery-ssc .block-main 提供（同開獎歷史、當前注項） */
.ssc-road {
  min-width: 0;
  display: flex;
