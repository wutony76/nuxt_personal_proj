...
// 每期 7 分鐘（server game/lottery/bg/base.ts timer.getStatusBySeconds）
const TIMELINE = [
        ...
        <div id="cd-section-play" class="rule-section">
          ...
          <ul class="rule-list rule-list-tight">
            ...
            <li><strong>號碼推薦</strong>：依「攪出次數 / 相隔期數」落差把 49 個號碼排序，
              ...
              一肖量顯示「5肖」、特碼顯示號碼球加兩面色波…），按「加入注項」加入的就是看到的那幾項。</li>
            <li><strong>推薦如何換算</strong>：五行／半波押特別號所屬那一組、七碼押對應的單雙顆數、
              一肖量押涵蓋的生肖數、連肖取預測會開出的生肖、連碼特串自動補上特別號；
              「不中」類玩法則反過來取<strong>最不被看好開出</strong>的號碼或生肖。
              換算與結算共用同一套判定邏輯，因此推出來的注項在該預測下必定中獎
              （少數分頁在特定排序下無必中組合，會退回最保險的一組並提示）。</li>
