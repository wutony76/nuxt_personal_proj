...
import { eggsTabOddsOf } from '#shared/config/eggscd/helpers'
 ...
 *
 * ⚠️ 與 ssc 的 Board 不同：eggs 的注項名稱本身就很短（大／小／極大／極小／豹子／
 *    對子／順子／紅波／藍波／綠波／0~27），不帶前綴、不需要 shortOf 削字。
 *    唯一要分辨的是「特碼」欄：0~9 是單一數字用號碼球，10~27 是兩位數改用方框，
 *    其餘文字玩法一律膠囊。
 */
const {
  state: mxState,
...
type BoardItem = EggsSelectItem & { odds: number }
...
const _handlers = {
  ...
  isSingleDigit: (name: string) => /^\d$/.test(String(name)),

  /**
   * 群組賠率摘要（同 k3 / ssc 的 oddsInfoOf）
   *   整組同賠率      → 賠率[ 2.06 ]
   *   賠率不同、注項少 → 逐項列出
   *   賠率不同、注項多 → 區間 + isRange（標題可 hover 浮出明細，例如特碼 28 項）
   */
  ...
  /** 橫向（row-major）矩陣：注項是有序的（特碼 0~27），必須依 config 順序橫向填 */
  ...
  /** 號碼欄要畫成什麼：號碼球（單一數字 0~9）／方框（其餘數字）／文字膠囊 */
  variantOf: (list: BoardItem[] = []) => {
    const names = list.map((item) => String(item?.name ?? ''))
    if (names.length > 0 && names.every((name) => _handlers.isSingleDigit(name))) return 'ball'
    return names.every((name) => _handlers.isNumber(name)) ? 'box' : 'pill'
  }
}
...
const layout = computed(() => {
  ...
  cloned.tabGroup.forEach((group: any) => {
    group.groupList.forEach((item: BoardItem) => {
      item.select = false
      item.coin = 0
      item.odds = eggsTabOddsOf(mxState.select, mxState.selectTabId, String(item.name))
      pool.push(item)
    })
  })
...
<template>
  <div class="eggs-board">
    ...
    <div v-for="group in tableGroups" :key="`eggs-group-${group.groupName}`" class="play-group">
      <div class="group-title">
        ...
        <span v-if="group.oddsSummary" class="group-odds" :class="{ 'has-detail': group.hasOddsDetail }"
          ...
          {{ group.oddsSummary }}
          <!-- 標題只顯示區間時（特碼），hover / focus 浮出逐項賠率 -->
            ...
            <template v-for="(item, colIndex) in row" :key="`cell-${group.groupName}-${rowIndex}-${colIndex}`">
              <td class="td-code" :class="[cellClassOf(item), { 'is-empty': !item }]" @click="click.cell(item)"
                ...
                <span v-if="item && group.variant === 'ball'" class="ball-set">
                  <Ball :digit="String(item.name)" size="md" />
                </span>
                <!-- 特碼 10~27（方框）與其餘玩法（膠囊） -->
