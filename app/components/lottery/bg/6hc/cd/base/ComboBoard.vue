...
/**
 * 組合型玩法看板（連碼 / 全不中 / 中一 / 特平中 共用）
 *
 * 與 PlayBoard（特碼／正碼／正碼特／七碼）的差別：那些玩法「一個注項 = 一注」，
 * 每個注項各自有金額輸入框；這裡是「選一組號碼 → 組成 N 注」，
 * 金額只有一個（每注同額），注數由 C(已選, pick) 決定，所以不能共用 PlayBoard。
 *
 * 適用條件（四個玩法都符合）：
 *   1. tabGroup 是號碼選取池（01~49），不是注項
 *   2. settings.combo 定義一注幾個號、複式上限
 *   3. 賠率放在 tiers（連碼多檔；其餘三個各只有一檔）
 *
...
const NUMBER_COLUMNS = computed(() => props.poolColumns ?? 10)
...
const numberRows = computed(() => _handlers.toMatrix((layout.value?.tabGroup?.[0]?.groupList ?? []) as PoolItem[], NUMBER_COLUMNS.value))
...
<template>
  <div class="main-play-base combo-board" :class="props.boardClass">
    ...
    <div class="play-group">
      ...
      <table class="play-table">
        <tbody>
          <tr v-for="(row, rowIndex) in numberRows" :key="`combo-row-${rowIndex}`">
            <td v-for="(item, colIndex) in row" :key="`combo-cell-${rowIndex}-${colIndex}`" class="td-code"
