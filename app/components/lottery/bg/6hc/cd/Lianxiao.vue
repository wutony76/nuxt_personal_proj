 ...
 * 連肖看板
 *
 * 結構與連碼／合肖同類：注項池由玩家自選（12 生肖選 n 個），選取後展開成 C(已選, pick) 組合，
 * 每個組合才是一注 —— 與 PlayBoard（特碼／正碼…）「一個注項 = 一注」不同，故不能共用。
 *
 * 與合肖的差別：合肖只看特別號一顆球（或的關係），連肖看整期 7 顆球、要求全部出現（且的關係，
 * 連中）或全部都不出現（連不中）。賠率同樣不是固定檔次表，取決於「所選的那幾個生肖」
 * （見 creditLianxiaoOddsOf），故不顯示 tier-bar，改即時依目前已展開的注項算出賠率區間。
 */
const NAME_COLUMNS = 6
...
// 注數 = 已選生肖可組出的組合數（= composable 展開後的當前注項數）
...
// 賠率取決於「所選的那幾個生肖」，非固定檔次表 —— 依目前展開的每個組合即時推算，
// 只選滿 1 組（= pick 個）時顯示單一賠率，選更多（複式展開多注）時顯示區間
const oddsRange = computed(() => {
  const year = new Date().getFullYear()
  const values = (mxSelect.items as ComboItem[])
    .map((item) => creditTabOddsOf(mxState.select, mxState.selectTabId, item.codes?.[0], year, item.codes))
    .filter((odds) => odds > 0)
...
<template>
  <div class="main-play-base lianxiao">
    ...
    <div class="quota-bar">
      ...
      <span v-if="mxCombo" class="quota-item">
        每注 {{ mxCombo.pick }} 個生肖 · 最多選 {{ mxCombo.maxPick }} 個（{{ mxMaxBets }} 注）
      </span>
      <span class="quota-note">※ 賠率依所選生肖即時計算，派彩以下注時鎖定的賠率為準</span>
    </div>

    <!-- 目前賠率（取決於所選的那幾個生肖，非固定值） -->
    ...
    <div class="play-group">
      ...
      <table class="play-table">
        <tbody>
          <tr v-for="(row, rowIndex) in nameRows" :key="`lianxiao-row-${rowIndex}`">
            <td v-for="(item, colIndex) in row" :key="`lianxiao-cell-${rowIndex}-${colIndex}`" class="td-code"
    ...
    <div class="combo-summary">
      <div class="summary-row">
        <span class="summary-label">已選生肖</span>
      ...
      <p v-if="shortfall > 0" class="summary-hint">
        還需再選 <strong>{{ shortfall }}</strong> 個生肖才能組成一注
      </p>
    </div>
  </div>
</template>
