...
const _handlers = {
  ...
  gameApi: (gameKey: RetroGameKey) => {
    ...
    if (gameKey === 'minesweeper') {
      return { history: api.games.retro.historyMinesweeper, record: api.games.retro.recordMinesweeper, clear: api.games.retro.clearMinesweeper }
    }
    if (gameKey === 'pacman') {
    ...
    if (gameKey === 'typing') {
      return { history: api.games.retro.historyTyping, record: api.games.retro.recordTyping, clear: api.games.retro.clearTyping }
    }
    if (gameKey === 'breakout') {
      return { history: api.games.retro.historyBreakout, record: api.games.retro.recordBreakout, clear: api.games.retro.clearBreakout }
    }
    if (gameKey === 'orbMatch') {
      return { history: api.games.retro.historyOrbMatch, record: api.games.retro.recordOrbMatch, clear: api.games.retro.clearOrbMatch }
    }
    if (gameKey === '2048') {
      return { history: api.games.retro.history2048, record: api.games.retro.record2048, clear: api.games.retro.clear2048 }
    }
    if (gameKey === 'flappy') {
      return { history: api.games.retro.historyFlappy, record: api.games.retro.recordFlappy, clear: api.games.retro.clearFlappy }
    }
    if (gameKey === 'frogger') {
      return { history: api.games.retro.historyFrogger, record: api.games.retro.recordFrogger, clear: api.games.retro.clearFrogger }
    }
    if (gameKey === 'connect4') {
      return { history: api.games.retro.historyConnect4, record: api.games.retro.recordConnect4, clear: api.games.retro.clearConnect4 }
    }
    if (gameKey === 'whackAMole') {
      return { history: api.games.retro.historyWhackAMole, record: api.games.retro.recordWhackAMole, clear: api.games.retro.clearWhackAMole }
    }
    if (gameKey === 'lightsOut') {
      return { history: api.games.retro.historyLightsOut, record: api.games.retro.recordLightsOut, clear: api.games.retro.clearLightsOut }
    }
    if (gameKey === 'towerStack') {
      return { history: api.games.retro.historyTowerStack, record: api.games.retro.recordTowerStack, clear: api.games.retro.clearTowerStack }
    }
    if (gameKey === 'arkanoid') {
