...
import { judgeKl10Bet, type Kl10BetResult ,
  ...
  KL10_POOL_PLAY_WEIGHT,
  KL10_POOL_BASE_MIN,
  KL10_POOL_BASE_MAX,
  KL10_POOL_RAKE_RATIO,
  KL10_POOL_QUOTA,
  KL10_POOL_PRIZE_TIERS,
  KL10_POOL_FLOOR
...
import { kl10QuotaOf, kl10RtpOf, kl10TabOddsOf, kl10HasBetCode, findKl10Tab , kl10JackpotWeightOf } from '#shared/config/kl10cd/helpers'

/**
 * 快樂十分（KL10）信用盤
 *
 * ── 與 PC蛋蛋（eggs.ts）的關係：同一套骨架 ───────────────
 *   來源（bglottery `kl10/`）只有信用模式、沒有官方盤（該資料夾沒有 official 子目錄），
 *   因此不需要 *Shared.ts 共用期表／彩池那一層，直接用 LOTTERY_BASE 內建的
 *   `handle.prdOpenCode()` 產生當日期表即可 —— 只覆寫 `randomOpenCode`
 *   （20 取 8 不重複）與 `openCodePlay`（球號展示），不必像 k3Cd 整個 prdOpenCode 重寫。
 *
 * ── 與其他彩種的差異 ────────────────────────────────────
 *   開獎是 1~20 取 8 個**不重複**號碼（洗牌，同 x5Cd 的做法），不是骰子也不是 49 取 7。
 *   ⚠️ 「任選」是一注多碼的組合玩法（注碼像 `任三中三03,07,15`），
 *      但複式展開在前端就做完了（見 app/composables/useKl10.ts），
...
type Group = {
  ...
  playList?: Array<{
    ...
    coin?: number | string
    /** 彩池玩法（選號）專用：一注多碼，不走 label 那套（見 KL10_POOL_PLAY_KEY 分支） */
...
type UserStoreLike = { userId?: string; coin?: number; kl10Record?: UserRecord }
...
/**
 * 取一注的注碼；注項名稱本身就是注碼
 * （"第一球07"、"第一球合單"、"龍虎12龍"、"總和大"、"上下和"、"任三中三03,07,15"）
 * ⚠️ 不像 eggs 可以退回 `num` —— 快樂十分的注碼一律帶前綴，光一個號碼不是合法注碼。
 */
function _resolveBetCode(play?: { label?: string | number }): string {
  return String(play?.label ?? '').trim()
}

/** 產生 8 顆球（1~20 不重複，洗牌後取前 8 個；格式一律補零兩位） */
function _kl10RandomOpenCode(): string[] {
  const source = Array.from({ length: KL10_NUMBER_MAX }, (_, i) => i + 1)
  ...
  return source.slice(0, KL10_BALL_COUNT).map((num) => kl10NumberLabel(num))
}

export default class KL10 extends LOTTERY_BASE {
  issueSettledMap: Record<string, boolean>
  /**
   * 各期爆池抽水累積：issue → 金額
   * ⚠️ 快樂十分沒有官方盤、沒有 Shared 層，只有這一個池 ——
  ...
  declare get: LOTTERY_BASE['get'] & {
    ...
    zoneOf: (openCode: string[]) => { sum: number; zone: string; parityZone: string }
    creditJackpot: () => {
      issue: string
    ...
    poolState: () => {
      ...
      issuePool: number
      distributable: number
      prizeTiers: typeof KL10_POOL_PRIZE_TIERS
    }
  }
  ...
  constructor() {
    ...
    Object.assign(this.handle, {
      // 快樂十分開獎：1~20 取 8 個不重複號碼
      randomOpenCode: () => _kl10RandomOpenCode(),
      // 開獎球資料：8 顆球都帶球位（正和／龍虎鬥都要靠球位判定）
      openCodePlay: (openCode: string[]) => {
        const nums = kl10NumbersOf(openCode)
        if (!nums) return []
        return nums.map((num, idx) => ({
          num,
          label: kl10NumberLabel(num),
          ball: idx + 1,
          index: idx
        }))
      },
      ensureUserRecord: (user: UserStoreLike) => {
        // 與其他彩種分開存（kl10Record），互不干擾
        if (!user.kl10Record) {
          user.kl10Record = { balanceChanges: [], betHistory: [], claimableIssues: [], updatedAt: Date.now() }
        }
        if (!Array.isArray(user.kl10Record.balanceChanges)) user.kl10Record.balanceChanges = []
        if (!Array.isArray(user.kl10Record.betHistory)) user.kl10Record.betHistory = []
        if (!Array.isArray(user.kl10Record.claimableIssues)) user.kl10Record.claimableIssues = []
        user.kl10Record.updatedAt = Date.now()
        return user.kl10Record
      },
      ...
      validateBetQuota: (input: { issue: string; userId: string; amount: number; groups: Group[] }) => {
        ...
        let poolNewCoin = 0
          ; (Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
            ...
            if (playKey === KL10_POOL_PLAY_KEY) {
              ; (Array.isArray(group?.playList) ? group.playList : []).forEach((play) => {
                const picks = kl10PoolPicksOf(Array.isArray(play?.codes) ? play.codes : [])
                if (!picks) {
                  this.handle.rejectBet(
                    `選號（彩池）號碼不合法，需 ${KL10_POOL_PICK_COUNT} 碼、每碼 1~20、不重複`
                  )
                }
                ...
                if (coin < KL10_POOL_QUOTA.item.min) {
                  this.handle.rejectBet(`選號（彩池）單注最低 ${_money(KL10_POOL_QUOTA.item.min)}，本次 ${_money(coin)}`)
                }
                if (coin > KL10_POOL_QUOTA.item.max) {
                  this.handle.rejectBet(`選號（彩池）單注上限 ${_money(KL10_POOL_QUOTA.item.max)}，本次 ${_money(coin)}`)
                }
                poolNewCoin += coin
              })
              return
              ...
              ; (Array.isArray(group?.playList) ? group.playList : []).forEach((play) => {
                ...
                const tabName = findKl10Tab(playKey, tabId)?.tabName ?? String(tabId)
                const betCode = _resolveBetCode(play)
                if (!betCode || !kl10HasBetCode(playKey, tabId, betCode)) {
                  this.handle.rejectBet(`${tabName}「${betCode || '(空白)'}」不是有效注項`)
                }
                const quota = kl10QuotaOf(playKey, tabId)
                const rawCoin = Number(play?.amount ?? play?.coin ?? input.amount)
                const coin = Number.isFinite(rawCoin) && rawCoin > 0 ? rawCoin : Number(input.amount)
        ...
        if (poolNewCoin > 0 && KL10_POOL_QUOTA.issue.max > 0) {
          ...
          if (used + poolNewCoin > KL10_POOL_QUOTA.issue.max) {
            this.handle.rejectBet(
              `選號（彩池） 單期投注上限 ${_money(KL10_POOL_QUOTA.issue.max)}，`
          ...
          ; (Array.isArray(input.groups) ? input.groups : []).forEach((group) => {
            ...
            playList.forEach((play, index) => {
              ...
              rows.push({
                ...
                odds: kl10TabOddsOf(playKey, tabId, betCode)
              })
            })
          })
        return rows
      ...
      settleIssuePrize: (issue: string, openCode: string[]) => {
        ...
        // 彩池玩法（選號）走獨立的分層結算，不能混進固定賠率的 judgeKl10Bet（picks 的裸數字無法辨識）
        const oddsOrders = issueOrders.filter((row) => String(row.playKey ?? '') !== KL10_POOL_PLAY_KEY)
        const poolOrders = issueOrders.filter((row) => String(row.playKey ?? '') === KL10_POOL_PLAY_KEY)
        ...
        oddsOrders.forEach((row) => {
          ...
          const judged = judgeKl10Bet(betCode, codes, coin, lockedOdds, kl10RtpOf(playKey, tabId))
          // 無法辨識的注項視為和局退還本金，避免吞掉玩家注金
          const result: Kl10BetResult = judged?.result ?? 'tie'
          ...
          jackpotRows.push({
            ...
            weight: kl10JackpotWeightOf(playKey, tabId, betCode)
          })
        })
        ...
        let carryPoolNext = 0
        KL10_POOL_PRIZE_TIERS.forEach((tier) => {
        ...
        poolRows.forEach((row) => {
          ...
          jackpotRows.push({
            ...
            weight: KL10_POOL_PLAY_WEIGHT
          })
        })
        this.carryPool = carryPoolNext
        this.issuePoolMap[safeIssue] = 0

        // ── 爆池發放（爆池期：奇偶一邊倒，見 KL10_JACKPOT_SETTINGS）──
        ...
        if (jackpot.triggered) {
          this.lastJackpotHit = {
            issue: safeIssue,
            openLabel: kl10JackpotLabel(codes),
            pool: jackpot.pool,
    ...
    Object.assign(this.get, {
      ...
      /**
       * 爆池狀態（供頁首與說明頁顯示）
       * ⚠️ 快樂十分只有這一個池，不像 k3 / pk10 / ssc 還要區分官方盤的共用彩池
       */
      creditJackpot: () => {
        ...
        return {
          ...
          rakeRatio: KL10_JACKPOT_SETTINGS.rakeRatio,
          payoutRatio: KL10_JACKPOT_SETTINGS.payoutRatio,
          minPool: KL10_JACKPOT_SETTINGS.minPool,
          hitLabel: KL10_JACKPOT_SETTINGS.hitLabel,
          hitRate: KL10_JACKPOT_SETTINGS.hitRate,
      ...
      /** 該期開獎的兩面特徵（總和／上下盤／奇偶盤），供看板與路珠顯示 */
      zoneOf: (openCode: string[]) => {
        const nums = kl10NumbersOf(openCode)
        if (!nums) return { sum: 0, zone: '', parityZone: '' }
        return { sum: kl10SumOf(nums), zone: kl10ZoneOf(nums), parityZone: kl10ParityZoneOf(nums) }
      },
      ...
      poolState: () => {
        ...
        return {
          ...
          prizeTiers: KL10_POOL_PRIZE_TIERS
        }
      }
    })
  ...
  playBets(payload: PlayBetsPayload, user: UserStoreLike) {
    ...
    // 抽水入爆池（快樂十分只有這一個爆池，沒有官方盤共用彩池）
    this.issueJackpotMap[issue] = Number((
      Number(this.issueJackpotMap[issue] ?? 0) + Number((amount * KL10_JACKPOT_SETTINGS.rakeRatio).toFixed(2))
    ).toFixed(2))
    // 同時抽水入彩池玩法的獨立彩金池（比照 K3-CD 兩條水並行，不限彩池玩法本身，任何分頁的下注都算）
    this.issuePoolMap[issue] = Number((
      Number(this.issuePoolMap[issue] ?? 0) + Number((amount * KL10_POOL_RAKE_RATIO).toFixed(2))
    ).toFixed(2))
