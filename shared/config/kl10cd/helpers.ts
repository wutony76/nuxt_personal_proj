/**
 * 快樂十分看板設定的讀取層（結構與 eggscd/helpers.ts 一致）
 *
 * 賠率與限額都以「分頁（tabId）設定」為主，取不到才退回全域預設；
 * 前端顯示 / clamp、伺端驗證 / 派彩全部走這裡，避免各自解析 config。
 *
 * ⚠️ 本檔 import kl10-cd.ts，因此 kl10-cd.ts 不可反向 import 本檔（會形成循環）。
 *    kl10-cd.ts 的判定需要設定值時一律由呼叫端傳入。
 *
 * ── 與 eggs / x5 那兩份最大的差異：任選是「一注多碼」 ──────────────
...
import { kl10ChanceOf, kl10KindOf, kl10OddsOf, KL10_RTP_FALLBACK } from '#shared/config/kl10-cd'

export type Kl10Quota = {
  /** 單注投注額 */
...
export type Kl10Chosen = { min: number; max: number; pick: number }

/** 分頁未設定 quota 時的預設值 */
export const KL10_QUOTA_FALLBACK: Kl10Quota = {
...
/** 取玩法設定（zhenghe / longhu / renxuan / liangmian） */
export function findKl10Play(playKey?: string): ConfigPlay | null {
...
export function findKl10Tab(playKey?: string, tabId?: number | string): ConfigTab | null {
  const play = findKl10Play(playKey)
...
/** 以注項名稱（或 playId）找設定項（靜態注項：正和／龍虎鬥／兩面） */
function _findTabItem(
  ...
  if (!code) return null
  const groups = findKl10Tab(playKey, tabId)?.tabGroup
 ...
 *    ⚠️ 前綴比對只在 `kl10KindOf(code) === 'renxuan'` 時才用，
 *       否則像「總和大」這種注碼可能被別的注項名稱前綴誤吃。
 */
function _resolveItem(
  ...
  const code = String(betCode ?? '').trim()
  if (!code || kl10KindOf(code) !== 'renxuan') return null
  const groups = findKl10Tab(playKey, tabId)?.tabGroup
  for (const group of Array.isArray(groups) ? groups : []) {
    const list = Array.isArray(group?.groupList) ? group.groupList : []
    const item = list.find((option) => {
...
/** 取分頁的投注限額 */
export function kl10QuotaOf(playKey?: string, tabId?: number | string): Kl10Quota {
  const quota = findKl10Tab(playKey, tabId)?.settings?.quota
  return {
    item: {
      min: _num(quota?.item?.min, KL10_QUOTA_FALLBACK.item.min),
      max: _num(quota?.item?.max, KL10_QUOTA_FALLBACK.item.max)
    },
    issue: { max: _num(quota?.issue?.max, KL10_QUOTA_FALLBACK.issue.max) }
  }
}
 ...
 */
export function kl10ChosenOf(playKey?: string, tabId?: number | string): Kl10Chosen | null {
  const chosen = findKl10Tab(playKey, tabId)?.settings?.chosen
...
/** 取分頁設定的回報率 */
export function kl10RtpOf(playKey?: string, tabId?: number | string): number {
  const rtp = Number(findKl10Tab(playKey, tabId)?.settings?.payout?.rtp)
  return Number.isFinite(rtp) && rtp > 0 ? rtp : KL10_RTP_FALLBACK
}

/** 取分頁設定的賠率上限（未設定回 0 表示不封頂） */
export function kl10MaxOddsOf(playKey?: string, tabId?: number | string): number {
  const maxOdds = Number(findKl10Tab(playKey, tabId)?.settings?.payout?.maxOdds)
 ...
 * 取注項賠率（含本金）
 *
 * 一律由 kl10-cd.ts 依「公平賠率 × 該分頁 rtp」推算，而不是讀 config 的 odds ——
 ...
 */
export function kl10TabOddsOf(playKey?: string, tabId?: number | string, betCode?: string | number): number {
  const found = _resolveItem(playKey, tabId, betCode)
  if (!found) return 0
  const odds = kl10OddsOf(String(betCode ?? ''), kl10RtpOf(playKey, tabId))
  if (!(odds > 0)) return 0
  const maxOdds = kl10MaxOddsOf(playKey, tabId)
 ...
 *    （`任三中三03,03,07` 也會前綴命中 `任三中三`），所以併驗 `kl10ChanceOf`。
 */
export function kl10HasBetCode(playKey?: string, tabId?: number | string, betCode?: string | number): boolean {
  if (_resolveItem(playKey, tabId, betCode) === null) return false
  return kl10ChanceOf(String(betCode ?? '')) !== null
}

/** 依 playId／name／任選前綴找到注項所在的 playKey/tabId（伺端結算只存得到 betCode 時用） */
export function kl10FindPlayLocation(betCode?: string | number): { playKey: string; tabId: number } | null {
 ...
 *    KL10_JACKPOT_SETTINGS.weightFallback）是兩件不同的事，故用 null 判斷而非 falsy。
 * @returns 權重；注項不在該分頁或無法辨識回 0
 */
export function kl10JackpotWeightOf(playKey?: string, tabId?: number | string, betCode?: string | number): number {
  const found = _resolveItem(playKey, tabId, betCode)
  if (!found) return 0
