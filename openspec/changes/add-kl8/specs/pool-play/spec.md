...
### Requirement: EGGS／KL10／KL8 SHALL 各提供一種彩池玩法分頁
系統 SHALL 在 `egg.vue`／`kl10.vue`／`kl8.vue` 提供一個獨立於既有固定賠率分頁的「選號（彩池）」
分頁，MUST NOT 出現在 `eggscd/plays.js`／`kl10cd/plays.js`／`kl8cd/plays.js` 的看板網格設定內
（比照 K3-OF 的 `xuanhao` 玩法，沒有對應的看板分頁）。

#### Scenario: 顯示彩池玩法分頁
- **WHEN** 使用者進入 `/lottery/bg/egg`、`/lottery/bg/kl10` 或 `/lottery/bg/kl8`
...
依命中顆數（4/3/2）對照 `KL10_POOL_PRIZE_TIERS` 分層派彩；KL8 彩池玩法 SHALL 為選 3 個號碼
（1~80，不重複），依命中顆數（3/2/1）對照 `KL8_POOL_PRIZE_TIERS` 分層派彩，其中命中 3 顆
（機率 1,140/82,160 ≈ 1.3874%）為頭獎、命中 2 顆（機率 ≈13.878%）為二獎、命中 1 顆
（機率 ≈43.086%）為三獎。`pool` 類型的層 SHALL 依「該層下注額比例」分配該層池額
（`totalPool × ratio`），頭獎 SHALL 有 `minAmount` 保底；`fixed` 類型的層 SHALL 為固定倍數。
...
#### Scenario: KL8 全中（3/3）
- **WHEN** 使用者選 3 個號碼，全部出現在當期 20 個開獎號中
- **THEN** 判定為頭獎，依 `KL8_POOL_PRIZE_TIERS` 頭獎層規則派彩
...
系統 SHALL 為 EGGS／KL10／KL8 各自維護一個獨立於既有爆池的彩金池，具備池底
（`EGGS_POOL_BASE_MIN/MAX`／`KL10_POOL_BASE_MIN/MAX`／`KL8_POOL_BASE_MIN/MAX`，隨機重骰）、
依全部注項下注額抽水（`XXX_POOL_RAKE_RATIO`）、未派出金額滾存至下期。此池 MUST NOT 與既有
爆池（`shared/config/jackpot.ts` 骨架）共用狀態或抽水來源；兩者可同時從同一筆下注各自抽水。
...
`KL8_POOL_PLAY_WEIGHT`）參與既有爆池（豹子／奇偶一邊倒／奇偶失衡）的分配，與一般固定賠率
注單一視同仁。
...
MUST NOT 只依賴換期輪詢才更新。KL8 於本次新增時 SHALL 直接依此要求實作，不得重現
EGGS／KL10 當初漏接即時刷新而需事後修正的問題。

#### Scenario: 下注後畫面立即反映新增的抽水
- **WHEN** 使用者在 EGGS、KL10 或 KL8 任一分頁成功送出下注
