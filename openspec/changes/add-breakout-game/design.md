...
- **關卡佈局圖案化**：目前 `buildBricks()`（[app/pages/game/breakout.vue:89-108](../../../../app/pages/game/breakout.vue#L89-L108)）每關都是固定 `BRICK_COLS`（10 欄）× 該關列數的滿版矩形，關卡難度只靠「列數變多」與「球速倍率提高」兩個變因驅動，沒有磚塊排列圖案（金字塔、留空洞、菱形等經典打磚塊常見花樣）的變化。若要加入，需要把 `LEVELS` 從 `{ rows, baseSpeedMul }` 擴充成可描述「每格是否有磚塊」的佈局資料（例如 `boolean[][]` 或字串圖樣），`buildBricks()` 依圖樣而非單純雙層迴圈生成磚塊。
- 屬於 Non-Goals 之外、規格未要求的加分項，優先度低於現有的球速/分數/coin 倍率校準工作。
