...
const { userRecord: mxRecord, wallet: mxWallet, isCd, fetch: mxFetch } = usePk10()
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="item in mxRecord.claimableIssues" :key="item.issue">
            ...
            <td>
              <span class="du-cars">
                <Car v-for="(code, idx) in (item.openCode ?? []).slice(0, 3)" :key="idx" :car="code" size="xs" />
              </span>
            </td>
        ...
        <tbody>
          <tr v-for="row in mxRecord.betHistory" :key="row.orderId">
            ...
            <td>
              ...
              <template v-else>{{ row.matchCount }} 個<em v-if="row.tierName">（{{ row.tierName }}）</em></template>
            </td>
            <td :class="`is-${row.winStatus}`">{{ statusText(row.winStatus) }}</td>
            <td>{{ row.winAmount > 0 ? money(row.winAmount) : '—' }}</td>
            <td>
              <span v-if="row.openCode?.length" class="du-cars">
                <Car v-for="(code, idx) in row.openCode.slice(0, 3)" :key="idx" :car="code" size="xs" />
              </span>
              <em v-else>—</em>
            </td>
          </tr>
