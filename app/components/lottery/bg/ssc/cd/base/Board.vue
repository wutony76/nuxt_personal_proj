...
import { useSsc, type SscSelectItem } from '~/composables/useSsc'
import { sscTabOddsOf } from '#shared/config/ssccd/helpers'
 ...
 *
 * ── 完全由 config 驅動 ──────────────────────────────────
 *   群組、注項、欄數、限額全部讀 shared/config/ssccd/plays.js（內容照 pcv2 提取），
 *   本元件不認得任何玩法名稱 —— 只看「注項名稱去掉前綴之後長什麼樣」決定畫法：
 *     單一數字（第一球7 → 7、全中3 → 3）→ 號碼球
 *     其餘數字                            → 方框
 *     文字（大／小／半順／龍／牛大…）      → 文字膠囊
 *   ⚠️ 與 pk10 不同：ssccd 的注項身上沒有 car / digit 之類的欄位，
 *      畫法只能從名稱推 —— 所以 shortOf 的前綴表就是這支元件的關鍵。
 *   賠率一律用 sscTabOddsOf 依該分頁 rtp 即時推算，不讀 config 的 odds 快照。
 */
const {
  state: mxState,
  select: mxSelect,
...
type BoardItem = SscSelectItem & { odds: number }
...
const _handlers = {
   ...
   *   群組「全5中1」        + 注碼「全中3」    → 3
   * ⚠️ 削完是空字串就退回原本的 name（例如群組「梭哈」+ 注碼「梭哈」這種假想情況），
   *    格子不能是空的。
  ...
  /**
   * 號碼欄要畫成什麼：號碼球（單一數字）／方框（其餘數字）／文字膠囊
  ...
  variantOf: (list: BoardItem[] = [], groupName = '') => {
    ...
    if (shorts.length > 0 && shorts.every((text) => /^\d$/.test(text))) return 'ball'
    return shorts.every((text) => _handlers.isNumber(text)) ? 'box' : 'pill'
  }
}
...
const layout = computed(() => {
  ...
  cloned.tabGroup.forEach((group: any) => {
    group.groupList.forEach((item: BoardItem) => {
      item.select = false
      item.coin = 0
      item.odds = sscTabOddsOf(mxState.select, mxState.selectTabId, String(item.name))
      pool.push(item)
    })
  })
...
<template>
  <div class="ssc-board">
    ...
    <div v-for="group in tableGroups" :key="`ssc-group-${group.groupName}`" class="play-group">
