...
import { K3_OF_PICK_COUNT } from '#shared/config/k3-of'
...
const {
  ...
  of: mxOf, ofPlayList, ofQuota, ofAutoCodes, ofCombo, isOfPool
} = useK3()

const money = (value: number) => Number(value ?? 0).toLocaleString('zh-TW')

/** 每注金額限額依當前分頁的 k3of settings.quota，超限伺端會整筆拒單 */
...
/** 注數上限；彩池玩法一次固定 1 注 */
const maxCount = computed(() => (isOfPool.value ? 1 : Math.max(1, poolSize.value || 1)))
...
const playInfo = computed(() => {
  ...
  if (isOfPool.value) return `${play}：隨機 ${K3_OF_PICK_COUNT} 個點數（固定 1 注）`
  const tab = mxOf.tabName || '-'
  if (ofCombo.value) {
    return `${play} / ${tab}：隨機挑點數組成 ${state.betCount} 注以上（上限 ${maxCount.value} 注）`
  }
