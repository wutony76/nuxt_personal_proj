...
/** 看板單選注項（定位膽用） */
export type Pl3SelectItem = { code: string; odds: number; coin: number }

const PLAYS = pl3Plays() as Array<{ key?: string; name?: string; list?: Array<{ tabId?: number; tabName?: string }> }>
...
const userRecord = reactive({
  ...
  betHistory: [] as Pl3UserBetHistory[],
...
// ── Computed ───────────────────────────────────────────────────────────────
const lotteryMeta = computed(() => LOTTERY.PL3)
...
/** 當前分頁限額 */
const currentQuota = computed(() => pl3QuotaOf(state.play, state.tabId))
/** 複式展開後的每一注（注碼字串） */
const comboCodes = computed(() => {
  if (!combo.value || isInputMode.value) return [] as string[]
  return pl3ComboCodes(state.play, state.tabId, board.picks)
})
 ...
 *
 * pl3ComboCodes() 超過 PL3_MAX_COMBO 會回空陣列，畫面沒辦法從空陣列分辨
 * 「還沒選滿」與「選太多」——所以自己算一份原始注數給提示用。
 * ⚠️ 三星組三是 n*(n-1)（有序取「哪個加倍/哪個不加倍」，非單純組合數 C(n,2)，比照 pl3-of.ts）。
 */
...
/** 是否因為超過上限而展不出注碼 */
const comboOverflow = computed(() => rawComboCount.value > PL3_MAX_COMBO)
/**
 * 複式／輸入模式還不能送單時的提示文案
 * @returns 可以送單時回空字串
 */
const comboHint = computed(() => {
  ...
  const c = combo.value
  if (!c) return ''
  if (comboOverflow.value) return `展開後超過 ${PL3_MAX_COMBO} 注，請縮小選號範圍`
 ...
 *   單選分頁 —— 該分頁的注碼數
 *   複式分頁 —— 全選展開的注數，夾到 PL3_MAX_COMBO
 *   輸入模式 —— 3 位數字全部組合數（1000），夾到 PL3_MAX_COMBO
 */
const autoMaxCount = computed(() => {
  const c = combo.value
  if (!c) {
    return (findPl3Tab(state.play, state.tabId)?.tabGroup ?? [])
      .flatMap((group: any) => group.groupList ?? []).length
  }
  if (c.mode === 'input') return Math.min(1000, PL3_MAX_COMBO)
  ...
  return Math.max(0, Math.min(total, PL3_MAX_COMBO))
})
 ...
 * ⚠️ 加太寬會超過 PL3_MAX_COMBO（pl3ComboCodes 回空陣列），一旦展不出來就停在上一組
...
function _widenPicks(pools: Array<Array<number | string>>, size: number): Array<Array<number | string>> | null {
  ...
  for (let take = start; take <= maxTake; take++) {
    ...
    const codes = pl3ComboCodes(state.play, state.tabId, picks)
...
const _actions = {
  ...
  setTab: (tabId: number | string) => {
    const tab = findPl3Tab(state.play, tabId)
    if (!tab) return
  ...
  toggleItem: (code: string) => {
    ...
    board.items.push({
      code: key,
      odds: pl3TabOddsOf(state.play, state.tabId, key),
   ...
   * ⚠️ 不需要像 useSsc 那樣依選項順序排序——pl3-of.ts 的複式展開函式（`_normalizeSets` /
   *    `pl3GroupCombos`）內部本來就會自己去重＋排序，前端送任意順序都一樣。
   */
  ...
  randomSelect: (count: number) => {
    ...
    if (!c) {
      const codes = groupList.value.length
        ? (findPl3Tab(state.play, state.tabId)?.tabGroup ?? [])
    ...
    if (!picks) return 0
    board.picks = picks
    return pl3ComboCodes(state.play, state.tabId, board.picks).length
  },
  /** 三星直選單式：更新輸入框內容並重新驗證 */
  ...
  oddsOf: (code: string) => pl3TabOddsOf(state.play, state.tabId, String(code ?? '')),
  clearBoard: () => {
    board.items = []
    board.input.text = ''
...
const fetch = {
  refreshCurrentInfo: async () => {
    try {
      const result = await api.lottery.currentPl3()
      if (!result) return
  ...
  userRecordAll: async () => {
    ...
    try {
      const res = await api.lottery.userRecordPl3()
  ...
  openCodeHistoryAll: async () => {
    ...
    try {
      const res = await api.lottery.openCodeHistoryPl3()
  ...
  claimOneIssue: async () => {
    ...
    try {
      const res = await api.lottery.claimOneIssuePl3()
      if (res?.ok) {
   ...
   *   複式分頁 —— 每格隨機挑，展開到注數 ≥ count（不衝破 PL3_MAX_COMBO）
  ...
  autoBets: async ({ count, amount }: { count: number; amount: number }) => {
    ...
    if (c?.mode === 'input') {
      ...
      const pools = Array.from({ length: _pickSlots() }, (_, pos) => _shuffle(_pickOptions(pos)))
      const picks = _widenPicks(pools, size)
      const codes = picks ? pl3ComboCodes(state.play, state.tabId, picks) : []
      if (codes.length === 0) return { ok: false, message: '此分頁無法自動選號', count: 0, amount: 0 }
      codes.forEach((code) => playList.push({ label: code, amount: coin }))
    } else {
      const codes = (findPl3Tab(state.play, state.tabId)?.tabGroup ?? [])
        .flatMap((group: any) => (group.groupList ?? []).map((option: any) => String(option?.name ?? '')))
        .filter((code: string) => code.length > 0)
...
export function usePl3() {
  return {
    ...
    fetch,
    /** 玩法定義（供玩法列表對帳） */
    playDefinitions: PL3_PLAY_DEFINITIONS
  }
}
