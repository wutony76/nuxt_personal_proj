 ...
 *   （命中3/2/1/0 → 1/27/243/729 每 1000）相同，故 `PL3_OF_PRIZE_TIERS` 直接沿用
 *   `SSC_OF_PRIZE_TIERS` 的既有比例，不重新設計。
 *   全站爆池比照 PC蛋蛋（EGGS）——PL3 與 EGGS 同為「單一盤口、無信用盤」，開獎結構相同
 *   （3 位 0~9 可重複），觸發條件（開豹子）與機率（1%）直接沿用 `EGGS_JACKPOT_SETTINGS`。
 *   詳見 openspec/changes/add-pl3-jackpot/design.md。
 *
 * ── 玩法（對照 bglottery pl3/config_ssc.js）─────────────
 ...
 *   ⚠️ 複式注數上限 PL3_MAX_COMBO，超過整筆拒絕，讓玩家自己縮小選號範圍。
 *
 * ⚠️ 本檔不可 import pl3of/helpers.ts（helpers 會 import 本檔，會形成循環）。
 */
import {
  pl3DigitsOf,
  pl3SumOf,
...
export const PL3_RTP_FALLBACK = 0.96
 ...
 */
export const PL3_MAX_COMBO = 2000

/** 判定結果：官方盤沒有和局，tie 只保留給無法辨識的注碼 */
export type Pl3OfBetResult = { status: 'win' | 'lose' | 'tie'; odds: number; payout: number }

/** 兩面 */
const SIDE_NAMES = ['大', '小', '單', '雙'] as const
type Pl3Side = (typeof SIDE_NAMES)[number]

/** 各玩法看的號碼位（0 起算）：百=0、十=1、個=2 */
export const PL3_OF_SECTIONS = {
  前二: [0, 1],
  後二: [1, 2],
  三星: [0, 1, 2]
} as const
export type Pl3OfSection = keyof typeof PL3_OF_SECTIONS

type Pl3OfBet =
  /** 定位膽：百位3（單一位置命中單一數字） */
  | { kind: 'ballPos'; place: number; digit: number }
  /** 位置直選：前二直選12 / 後二直選12 / 三星直選123 */
  | { kind: 'direct'; section: Pl3OfSection; digits: number[] }
  /** 前二/後二組選：2 碼互異、不計順序 */
  | { kind: 'group2'; section: Pl3OfSection; digits: [number, number] }
  ...
  /** 大小單雙：前二/後二每個位置一個面 */
  | { kind: 'sides'; section: Pl3OfSection; sides: Pl3Side[] }

/** 位置直選的注碼前綴 → 區段 */
const DIRECT_PREFIX: Record<string, Pl3OfSection> = {
  前二直選: '前二',
  後二直選: '後二',
  三星直選: '三星'
}
/** 前二/後二組選的注碼前綴 → 區段 */
const GROUP2_PREFIX: Record<string, Pl3OfSection> = {
  前二組選: '前二',
  後二組選: '後二'
}
 ...
 */
function _parseBet(betCode: string | number): Pl3OfBet | null {
  ...
  if (code.startsWith('大小單雙')) {
    ...
    const section = (['前二', '後二'] as Pl3OfSection[]).find((s) => rest.startsWith(s))
    if (!section) return null
    const tail = rest.slice(section.length)
    if (tail.length !== PL3_OF_SECTIONS[section].length) return null
    const sides = tail.split('').map((ch) => SIDE_NAMES.find((s) => s === ch))
    if (sides.some((s) => !s)) return null
    return { kind: 'sides', section, sides: sides as Pl3Side[] }
  }
  ...
  if (code.startsWith('三星直選和值')) {
    ...
    if (sum < PL3_SUM_MIN || sum > PL3_SUM_MAX) return null
  ...
  if (code.startsWith('三星組選和值')) {
    ...
    if (sum < 1 || sum > PL3_SUM_MAX - 1) return null
  ...
  for (const [prefix, section] of Object.entries(DIRECT_PREFIX)) {
    ...
    const digits = _digitsOf(code.slice(prefix.length), PL3_OF_SECTIONS[section].length)
  ...
  // ── 定位膽（百位3 / 十位5 / 個位9）──
  for (const name of PL3_PLACE_NAMES) {
    if (!code.startsWith(name)) continue
    const rest = code.slice(name.length)
    if (!/^\d$/.test(rest)) return null
    return { kind: 'ballPos', place: PL3_PLACE_NAMES.indexOf(name), digit: Number(rest) }
  }

  return null
}

const _sideHit = (side: Pl3Side, value: number): boolean => {
 ...
 */
export function pl3ChanceOf(betCode: string | number): Pl3Chance | null {
  const bet = _parseBet(betCode)
  if (!bet) return null
  const D = PL3_DIGIT_MAX + 1 // 10
  const T = PL3_TOTAL_OUTCOMES // 1000
  switch (bet.kind) {
    ...
    case 'direct':
      return { hit: 1, total: D ** PL3_OF_SECTIONS[bet.section].length }
    ...
    case 'sumValue':
      return { hit: pl3SumCounts()[bet.sum] ?? 0, total: T }
    // 三星組選和值：命中數查 ZUSHZ 排列數表（排除豹子，該表加總 990），母數仍是全部 1000
    case 'groupSumValue':
      return { hit: pl3GroupSumCounts()[bet.sum] ?? 0, total: T }
    // 一碼不定位：至少一位是該碼 = 1000 − 9³ = 271（與所選碼無關）
    case 'unpositioned1':
      return { hit: T - PL3_DIGIT_MAX ** 3, total: T }
    // 二碼不定位：兩碼各至少出現一次（排容）= 1000 − 2·9³ + 8³ = 54（與所選碼無關）
    case 'unpositioned2':
      return { hit: T - 2 * PL3_DIGIT_MAX ** 3 + (PL3_DIGIT_MAX - 1) ** 3, total: T }
 ...
 */
export function pl3IsHit(betCode: string | number, openCode: Array<string | number>): boolean | null {
  const digits = pl3DigitsOf(openCode)
  if (!digits) return null
  const bet = _parseBet(betCode)
  if (!bet) return null

  const pick = (section: Pl3OfSection) => PL3_OF_SECTIONS[section].map((i) => Number(digits[i]))
  ...
  switch (bet.kind) {
    ...
    case 'sumValue':
      return pl3SumOf(digits) === bet.sum
    case 'groupSumValue':
      // 排除豹子：豹子即使和值對上也不算中組選和值
      return !pl3IsTriple(digits) && pl3SumOf(digits) === bet.sum
    case 'unpositioned1':
...
/** 注碼種類；無法辨識回 null */
export function pl3KindOf(betCode: string | number): Pl3OfBet['kind'] | null {
 ...
 */
export function pl3OddsOf(betCode: string | number, rtp: number = PL3_RTP_FALLBACK): number {
  const chance = pl3ChanceOf(betCode)
  if (!chance || !(chance.hit > 0) || !(chance.total > 0)) return 0
  const safeRtp = Number.isFinite(rtp) && rtp > 0 ? rtp : PL3_RTP_FALLBACK
 ...
 */
export function judgePl3Bet(
  betCode: string,
  openCode: Array<string | number>,
  coin = 1,
  lockedOdds = 0
): Pl3OfBetResult | null {
  const hit = pl3IsHit(betCode, openCode)
  if (hit === null) return null
  const odds = lockedOdds > 0 ? Number(lockedOdds) : pl3OddsOf(betCode)
...
function _normalizeSets(sets: Array<Array<number | string>>): number[][] | null {
  const lists = (Array.isArray(sets) ? sets : []).map((list) => {
    const nums = (Array.isArray(list) ? list : [])
      .map((n) => Number(n))
      .filter((n) => Number.isInteger(n) && n >= 0 && n <= PL3_DIGIT_MAX)
 ...
 * 位置型複式展開（前二/後二/三星直選）
 *
 * ⚠️ 排列3 的號碼可以重複，所以**不濾掉重複組合**（與 pk10 的名次排列不同）。
 * @returns 每一注的號碼陣列；任一位沒選或超過上限回空陣列
 */
export function pl3DirectCombos(sets: Array<Array<number | string>>): number[][] {
  const lists = _normalizeSets(sets)
  if (!lists) return []
  const size = lists.reduce((acc, l) => acc * l.length, 1)
  if (size > PL3_MAX_COMBO) return []
  let combos: number[][] = [[]]
  lists.forEach((list) => {
    const next: number[][] = []
...
export function pl3SideCombos(sets: Array<Array<string>>): string[][] {
  const lists = (Array.isArray(sets) ? sets : []).map((list) =>
    Array.from(new Set((Array.isArray(list) ? list : []).filter((s) => SIDE_NAMES.includes(s as Pl3Side))))
  )
 ...
 */
export function pl3GroupCombos(
  ...
  return out.length > PL3_MAX_COMBO ? [] : out
}

/** 玩法定義（順序即前端玩法列的顯示順序，需與 pl3of/plays.js 一致） */
export const PL3_PLAY_DEFINITIONS: Array<{ key: string; name: string }> = [
...
export { PL3_DIGIT_MAX, PL3_PLACE_NAMES, PL3_TOTAL_OUTCOMES }
...
export const PL3_POOL_PLAY_KEY = 'sanxing'
 ...
 *    詳見 design.md 決策 3。
 */
export const PL3_OF_RAKE_RATIO = 0.6

/** 三星直選池底範圍（比照 SSC-OF 的 120,000~480,000，因三星直選池比照 SSC-OF 完全吃池） */
export const PL3_POOL_BASE_MIN = 120_000
export const PL3_POOL_BASE_MAX = 480_000

/** 池底重骰門檻：可派發金額低於「頭獎保底 ÷ 頭獎比例」時視為不足 */
export const PL3_POOL_FLOOR = (() => {
  const top = PL3_OF_PRIZE_TIERS.find((tier) => tier.type === 'pool' && tier.minAmount !== undefined)
 ...
 */
export function pl3SanxingPicksOf(betCode: string | number | Array<string | number>): number[] | null {
  ...
  if (picks.some((digit) => !Number.isInteger(digit) || digit < 0 || digit > PL3_DIGIT_MAX)) return null
  return picks
}
 ...
 */
export function pl3SanxingMatchCount(
  betCode: string | number | Array<string | number>,
  openCode: Array<string | number>
): number | null {
  const picks = pl3SanxingPicksOf(betCode)
  const digits = pl3DigitsOf(openCode)
...
/** 命中位數 → 所屬分層；不中回 null */
export function pl3SanxingTierOf(matchCount: number): Pl3OfPrizeTier | null {
  const count = Number(matchCount)
  if (!Number.isInteger(count) || count <= 0) return null
  return PL3_OF_PRIZE_TIERS.find((tier) => tier.match === count) ?? null
}
 ...
 */
export function pl3SanxingMatchCounts(picks: number[] = [1, 2, 3]): number[] {
  const table = [0, 0, 0, 0]
  const max = PL3_DIGIT_MAX + 1
...
/**
 * 全站爆池設定（沿用 EGGS_JACKPOT_SETTINGS 數值：PL3 與 EGGS 同為單一盤口、開獎結構相同）
 * ⚠️ boardWeight 只列 of —— PL3 沒有信用盤，這個係數在這裡等於不作用。
 */
export const PL3_JACKPOT_SETTINGS: JackpotSettings = {
  rakeRatio: 0.01,
  payoutRatio: 0.5,
  minPool: 1000,
  weightFallback: 1,
...
export const PL3_JACKPOT_BASE_MIN = 110_000
export const PL3_JACKPOT_BASE_MAX = 450_000

/**
 * 這一期是不是爆池期（開出豹子）
 * @returns true = 豹子；開獎格式不合回 false
 */
export function pl3JackpotHit(openCode: Array<string | number>): boolean {
  const digits = pl3DigitsOf(openCode)
  if (!digits) return false
  return pl3IsTriple(digits)
}

/** 爆池期的開獎文字（寫進爆池紀錄，給看板顯示用） */
export function pl3JackpotLabel(openCode: Array<string | number>): string {
  const digits = pl3DigitsOf(openCode)
  if (!digits) return ''
