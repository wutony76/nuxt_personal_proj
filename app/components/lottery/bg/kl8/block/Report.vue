...
/**
 * 下注紀錄（含結算結果）與可領獎金
 * 版面與捲動行為參照 kl10 的 Report.vue（表格區吃剩餘高度、內部捲動，分頁器固定下方）
 */
const { userRecord: mxRecord, fetch: mxFetch } = useKl8()
...
<template>
  <section class="block-main kl8-report">
    <div class="rp-head">
...
/* 卡片外框由全域 .lottery-kl8 .block-main 提供；表格沿用全域 .report-table（同 kl10） */
.kl8-report {
  flex: 1 1 auto;
  min-width: 0;
  min-height: 0;
