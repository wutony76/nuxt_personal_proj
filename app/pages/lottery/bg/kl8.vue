...
import Kl8Header from '~/components/lottery/bg/kl8/block/Header.vue'
import Kl8Board from '~/components/lottery/bg/kl8/base/Board.vue'
import Kl8BoardRenxuan from '~/components/lottery/bg/kl8/base/BoardRenxuan.vue'
import Kl8PoolPicker from '~/components/lottery/bg/kl8/block/PoolPicker.vue'
import CurrItems from '~/components/lottery/bg/kl8/block/CurrItems.vue'
import Controls from '~/components/lottery/bg/kl8/block/Controls.vue'
import Report from '~/components/lottery/bg/kl8/block/Report.vue'
import History from '~/components/lottery/bg/kl8/block/History.vue'
import Road from '~/components/lottery/bg/kl8/block/Road.vue'
import DialogUser from '~/components/lottery/bg/kl8/block/DialogUser.vue'
import DialogOpenCode from '~/components/lottery/bg/kl8/block/DialogOpenCode.vue'
import DialogRule from '~/components/lottery/bg/kl8/block/DialogRule.vue'
 ...
 * 快樂8（KL8）信用玩法
 *
 * 版面骨架與快樂十分（`kl10.vue`）／PC蛋蛋（`egg.vue`）同一套：單一盤口、單一頁面、單一爆池。
 * 注項一律讀 shared/config/kl8cd 的設定檔，賠率由 helpers 依該分頁 rtp 即時推算，
 * 畫面顯示與伺端鎖進注單的值一致。
 * ⚠️ 快樂8只有「任選」「兩面」兩個玩法（沒有 kl10 的正和／龍虎鬥），
 *    外加一個獨立的「選號（彩池）」玩法：
 *    - 任選是選號池 + 複式展開（`BoardRenxuan.vue`）
 *    - 選號彩池是固定選 3 碼 + 分層派彩（`PoolPicker.vue`）
 *    兩者與「一注項一金額」的表格看板互動不同，所以看板與機選按鈕都要依 `isRenxuan`／`isPoolPlay` 分流。
 */
const {
  state: mxState,
  wallet: mxWallet,
...
/** 下方的自動下注／CHAT 面板由 app.vue 的 BgAutoPanel 統一渲染（同 kl10） */
...
/**
 * 未登入處理：與 kl10 一致 —— 先 await useAuth().init() 確認登入狀態，
...
<template>
  <div class="base lottery-kl8">
    ...
    <main class="main">
      <Kl8Header @open-opencode-dialog="dialogClick.openOpenCode()" />
        ...
        <div class="selector-warp">
          <div class="selector">
            ...
            <div class="body">
              ...
              <Kl8BoardRenxuan v-if="isRenxuan" />
              <Kl8PoolPicker v-else-if="isPoolPlay" />
              <Kl8Board v-else />
            </div>
          </div>
...
<style lang="scss">
/* ── 版面骨架：與 kl10 同一套（容器寬度、卡片邊框陰影、進場動畫）───────── */
.lottery-kl8 {
  min-height: 100vh;
  display: flex;
  flex-direction: column;

  /* 背景上升光點（同 kl10 的 .bg-fx，keyframes 另立 kl8- 前綴避免依賴他頁） */
  .bg-fx {
    position: fixed;
    inset: 0;
    pointer-events: none;
    ...
    .orb {
      ...
      animation: kl8-orb-rise calc(7s + var(--i) * 0.6s) ease-in infinite;
  ...
  .info-warp {
    ...
    gap: 0.75rem;
    align-items: stretch;
    animation: kl8-sec-in 0.55s ease both;
    ...
    .info-side {
      ...
      .user-warp {
        ...
        cursor: pointer;
        animation: kl8-card-glow 3.5s ease-in-out infinite;
