 ...
 *   來源（bglottery fc3d）只有官方盤、沒有信用盤，所以沒有 mode / setMode、沒有信用盤選號。
 *   三星直選（複式＋單式）已改吃分層彩池、全站另有開豹子觸發的爆池（見
 *   openspec/changes/add-fc3d-jackpot/），故仍比照 useEggs 提供 `creditJackpot`／
 *   `poolPlayState` 兩份唯讀狀態；但 FC3D 沒有 EGGS 那種獨立選號側玩法（xuanhao），
...
const isPoolTab = computed(() => fc3dOfIsPoolTab(state.play, state.tabId))
/** 複式分頁每個位置可選的號碼／和值／面 */
...
const fetch = {
  ...
  creditJackpot: async () => {
    try {
      Object.assign(creditJackpot, await api.lottery.jackpotFc3d())
    } catch { /* 爆池取不到不阻斷畫面 */ }
  },
  /** 三星直選分層彩池狀態（比照 useEggs 的 fetch.poolState） */
  poolState: async () => {
    try {
      Object.assign(poolPlayState, await api.lottery.poolFc3d())
    } catch { /* 彩池取不到不阻斷畫面 */ }
  },
  claimOneIssue: async () => {
