 ...
 * 快樂8頁首
 *
 * 版面參照 kl10 / k3 的 Header：
 *   .header-warp  4px 深紅外框 + 金色漸層頂條
 *   .left         標題 + LOTTERY_ID + 爆池獎金框
 *   .right .inner .timer（期別／狀態／倒數）＋ .open-code（可點的開獎區）
 *
 * ── 與 kl10 的差異 ────────────────────────────────────────
 *   開獎球是 20 顆（1~80，不重複），兩面標示除了總和大小／單雙、上下盤、奇偶盤，
 *   還多一個「五行」（依 20 球總和落在等機率五等分區間）。爆池條件看奇偶一邊倒。
 */
const emit = defineEmits<{ (event: 'open-opencode-dialog'): void }>()

const { current: mxCurrent, creditJackpot: mxJackpot, time: mxTime, lotteryMeta } = useKl8()
 ...
 * 彩池（獎金框）
 *
 * 版位與 kl10 頁首的 `.info-bonus` 相同（總額／預估／機率三列 + 明細），三列全部讀爆池：
 *   總彩池   = 爆池可發放額（當期抽水 + 未發放滾存）
 *   預估發放 = 現在就觸發的話會發出多少（可發放額 × payoutRatio）
 *   觸發機率 = 爆池條件的發生機率（hitRate）
 * ⚠️ 快樂8沒有官方盤、沒有共用彩池，這是唯一的池；另有一條獨立的「選號彩池」在投注區顯示。
 */
...
/**
 * 池額跳動動畫（與 kl10 / k3 / 6hc-of 的 Header 同一套）
 *
...
const PLACEHOLDER_BALLS = Array.from({ length: KL8_BALL_COUNT }, () => 0)
const balls = computed(() => (openCode.value.length > 0 ? openCode.value : PLACEHOLDER_BALLS))
const nums = computed(() => kl8NumbersOf(openCode.value))
const sum = computed(() => (nums.value ? kl8SumOf(nums.value) : 0))
const isBig = computed(() => sum.value >= KL8_SUM_BIG_LINE)
const isOdd = computed(() => sum.value % 2 === 1)
/** 上下盤／奇偶盤（和盤在這裡如實顯示「和盤」，注碼才寫成 上下和／奇偶和） */
const zone = computed(() => (nums.value ? kl8ZoneOf(nums.value) : ''))
const parityZone = computed(() => (nums.value ? kl8ParityZoneOf(nums.value) : ''))
/** 五行（依 20 球總和落在等機率五等分區間） */
const wuxing = computed(() => (nums.value ? kl8WuxingOf(sum.value) : ''))

/**
 * 開獎動畫
 *   準備開獎／開獎中 → 20 顆球持續翻號並抖動（rolling）
  ...
  start: () => {
    ...
    rollTimer = setInterval(() => {
      ...
      anim.rollFaces = PLACEHOLDER_BALLS.map(
        () => KL8_NUMBER_MIN + Math.floor(Math.random() * (KL8_NUMBER_MAX - KL8_NUMBER_MIN + 1))
      )
    }, ROLL_INTERVAL_MS)
  },
  stop: () => {
...
<template>
  <header class="header-warp">
    <div class="left">
      <div class="info">
        <h1 class="title">快樂8</h1>
        <p class="sub">信用玩法</p>
        <p class="lotteryId">LOTTERY_ID: {{ lotteryMeta.id }}</p>
      </div>

      <!-- 獎金框：版位同 kl10，三列讀的是爆池（快樂8沒有官方盤共用彩池） -->
        ...
        <div class="open-code" role="button" tabindex="0" @click="emit('open-opencode-dialog')"
          ...
          <div class="main">
            ...
            <div v-if="openCode.length > 0 && !anim.rolling" :key="`kl8-meta-${anim.revealToken}`" class="open-meta">
              ...
              <span v-if="wuxing" class="meta-tag is-wuxing">{{ wuxing }}</span>
            </div>
          </div>
        </div>
      </div>
...
/* 樣式沿用 kl10 的 .header-warp，開獎球換成 20 顆、兩面標示多一個五行 */
.header-warp {
  display: flex;
  flex-direction: row;
  align-items: stretch;
  ...
  .left {
    ...
    .info {
      ...
      .title {
        margin: 0;
        /* 「樂」不在 Ma Shan Zheng 的實際字圖裡，換 LXGW WenKai TC 避免同一標題缺字混字體 */
    ...
    /* 獎金框：樣式沿用 kl10 的 .info-bonus（固定寬度、不跟右側搶空間） */
    .info-bonus {
      flex: 0 0 250px;
      display: grid;
      gap: 0.375rem;
      ...
      .open-code {
        ...
        .main {
          ...
          .ball-warp {
            display: inline-flex;
            align-items: center;
            animation: kl8-ball-in 0.5s cubic-bezier(0.2, 0.9, 0.3, 1.3) both;
            animation-delay: calc(var(--i, 0) * 0.05s);

            &.is-rolling {
              animation: kl8-ball-roll 0.32s linear infinite;
