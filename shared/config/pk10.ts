 ...
 *   前一直選（OF）         前一01 ~ 前一10                    母數 10
 *   前二直選（OF）         前二05-03（冠軍 05、亞軍 03）       母數 90
 *   ⚠️ 前三直選不走本檔的賠率 —— 它是彩池分層玩法，見 shared/config/pk10-of.ts。
 *
 * ── 與 k3 的分工差異 ────────────────────────────────────
 *   快3 的信用盤（k3-cd.ts）與官方盤賠率制（k3of.ts）注碼表本來就不同，故各寫一份判定。
 *   PK10 兩個盤口的注碼語意完全一樣（差別只在 rtp、限額與哪些玩法開放），
 *   因此判定與機率統一收在本檔，避免兩份互相飄移：
 *     shared/config/pk10-cd.ts  信用盤：判定包裝 + 賠率推導
 *     shared/config/pk10of.ts   官方盤賠率制：同上，但 rtp 預設值自己一份
