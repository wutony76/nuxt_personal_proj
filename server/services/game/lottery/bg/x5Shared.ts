 ...
 * ⚠️ 範圍沿用 X5_POOL_BASE_MIN/MAX（共用彩池的既有池底範圍），只是套用在爆池這個獨立的池上，
 *    兩個池底互不影響、各自隨機。
 */
export const X5_JACKPOT_BASE_MIN = X5_POOL_BASE_MIN
export const X5_JACKPOT_BASE_MAX = X5_POOL_BASE_MAX
...
export function x5RegisterJackpotBoard(board: X5JackpotBoard): void {
  ...
  _jackpotSeeded = true
  X5_JACKPOT.carry = LOTTERY_BASE.jackpotBase(X5_JACKPOT_BASE_MIN, X5_JACKPOT_BASE_MAX)
}
