...
/**
 * 當前注項：已選注項列表，金額可直接改（快樂8只有信用盤，沒有官方盤那一分支）
 *
 * ⚠️ 任選分頁沒有「已選注項」可改金額 —— 它是選號後才展開成 C(k, N) 注、共用同一個單注金額，
 *    所以那個模式改成唯讀列出展開後的注碼（金額在看板／控制列上改）。彩池玩法同理。
 */
const {
...
const renxuanRows = computed(() =>
  renxuanCombos.value.map((combo) => ({
    key: combo.join('-'),
    code: `${mxState.selectTabName}${combo.map((num) => kl8NumberLabel(num)).join(',')}`,
...
const poolRows = computed(() => {
  ...
  return [{
    ...
    code: `選號（彩池）${poolPlayPicked.value.map((num) => kl8NumberLabel(num)).join(',')}`,
...
const click = {
   ...
   */
  coinInput: (item: Kl8SelectItem, event: Event) => {
  ...
  coinBlur: (item: Kl8SelectItem, event: Event) => {
    const target = event.target as HTMLInputElement
...
<template>
  <section class="block-main kl8-curr">
    <div class="curr-head">
...
/* 卡片外框由全域 .lottery-kl8 .block-main 提供，這裡只寫內容樣式（同 kl10） */
.kl8-curr {
  height: 350px;
  display: flex;
